...
rungs named, never detailed —
a phase gains internal granularity only once its predecessor lands,
because the groom that details it holds learnings the cut could not.
Each rung's done-line is a STATE predicate — a condition to sight,
never an adjective and never an ordinal (which a reorder silently redefines).
This heat executes the near rung only; the far rung belongs to the next heat in the chain.

## Entry gate

Both must hold before any founding-and-cutover pace is slated or mounted:

- The scratch-studbook rehearsal checklist sighted green:
...
### Rung 1 (near, this heat): founding-and-cutover

Done when: every install reads and writes heat/pace state through the studbook alone —
both readers repointed (every `jjx_*` command's read AND the dispatch spine's target-resolution read) —
the old in-repo gallops is a tombstone old binaries fail loud on,
consumer repos carry no JJK insertions (CLAUDE.md includes, `.claude/jjm`, MCP config),
...
the disposable-scaffolding ruling on the standing scratch instance,
and anticipating the first-contact trust prompt (finding C).

### Rung 2 (far, the next heat): footprint-delivery — ONE rung

Done when: cross-station work on one sire is seamless and the footprint lands clean —
a second station adopts, never forks, billet work another station pushed,
and work products reach the sire in its footprint posture's tolerated form
while knowledge products stay in JJ's stores.

ONE rung deliberately: the post-cutover groom decides its internal granularity,
holding cutover learnings this cut cannot.
Its seed is the seam enumeration of finding B's cross-machine coherence cluster,
carried verbatim below and transcribed forward by the terminal pace —
the groom must consciously rung-or-collapse EACH seam, never skim a paragraph:
...
The 260709 footprint-posture and engagement-inversion cinches carried below are this rung's
defining context — they have no spec home yet.

## Carry-forward discipline

Each phase boundary is a SLATED carry-forward pace, never paddock prose:
it transcribes the live findings forward VERBATIM
(copied intact, never re-glossed — a paraphrase is where knowledge leaks across a hop)
and expands the next phase into paces.
A slated pace shows in the roster and blocks a clean retirement; prose does not.
This heat's terminal pace is that boundary:
it cuts the footprint-delivery heat,
and that heat's paddock must restate this discipline for its own carry-forward pace —
or record that the ladder ends there.

## Carried forward from the build heat (₣Br), verbatim at cut time (260714)

Frozen record, copied intact from the build heat's paddock so it is never read from a retired paddock.
Deictic words inside ("this heat", "this session") speak from ₣Br;
coronets inside are record-of-the-moment, never live references.

### Finding B and the cross-machine coherence cluster (260714 rehearsal)
...
### Footprint-delivery context (260709 cinches, no spec home yet)
...
and the footprint-delivery heat stands —
cut by this heat's terminal carry-forward pace with the seed enumeration
and every live finding transcribed verbatim into it.