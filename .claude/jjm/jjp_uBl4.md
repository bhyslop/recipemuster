## Paddock: rbk-07-implement-matricula

## Character
Implementation, not design — the aspirant is settled in the spec; build to it.
The one drift to resist: re-opening the deferred set (the post-MVP widening, the grimoire) that ₣Bg consciously fenced out.
...
This paddock is shape, not a restatement — build from the spec.
The matricula is an additive sibling to the vof library (Tools/vok/vof/src/vofc_registry.rs), reusing it as a path dependency: the cipher registry is the input grammar, the release tree-walk generalizes into the candidate gate, and emission rides the voff managed-section rail.

## Cinched (inherited, hard)
Transient by design (VOr_k3p): no persistent census, re-derived each call, discarded; only the small stable grammar is maintained.
Mechanical, never linguistic (VOr_m7w): set operations over inscriptions only — no word generation, no fuzzy match; sprue/rivet checks stay advisory, never assertions.
...
v1 scan scope: quoins-and-signets — the cinched MVP; sprue and rivet coverage stays advisory per VOr_m7w.
Classifier growth: bootstrap from the cipher gate plus a few classifiers, residue-driven — the estray set drives which classifier to write next.
Declaration-recognition fidelity: per-Vesture line patterns for the MVP (leaning on RCG/BCG declaration-site discipline); tree-sitter adopted incrementally only where a Vesture's structure defeats a regex — never syn.
Rivet recognition is in-MVP only far enough that opaque rivet IDs (RBr_/VOr_/JJr_) are seen as their own kind and do not fall into the estray pile as false product; full sprue-domain coverage defers.

## Open inside the fence (answered while building, not before)
