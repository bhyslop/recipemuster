## Charter — produce the release

Take the delivery candidate the staging heat (₣Bs) proved,
re-cut under this heat's amended ceremony so it sits one commit atop the live public base,
and carry it to a live public release:
stage it on the public repository, walk it as a stranger, promote it to production, and prove the promoted face.
This is the first real content delivery of the federation era — the heat exists so the tectonic step is a sequence of small proven ceremonies, not one leap.
...
The candidate stands promoted to the public repository's main with tree-hash equality proven;
the yawp/README links live and the anchors resolving at the published URL;
the production landing spot-check green (repo front page, github.io, fresh default-branch clone into the onboarding entry);
the divergence census from the walk dispositioned;
the ceremony document carries the structural-versus-liveness link-check split and records this heat's shape as the delivery pattern;
the staging branch deleted post-promotion;
and the release is recorded per the ceremony document.