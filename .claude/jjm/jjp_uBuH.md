...
**Why GAR's `package` is the universal substrate primitive.** Both Hallmark (built side) and {INSTANCE_NOUN} (captured side) manifest in GAR as a `package` containing image + about + vouch (+ manifest-of-refs for compound Lodes). This equivalence is structural, not coincidental — it is what made the built-side grouping noun (formerly debated as "ark") load-bearing in the first place, closing what was ₣Az's "remove ark" debt. The cross-side consequence: registry-level operations (delete, presence-probe, byte-fetch) share one implementation backbone in the registry-utility layer, called from each side's cult verbs.

**Why registry-level operations get parallel cult verbs per side, not shared verbs.** Built-side `abjure` and the captured-side delete operation share one registry primitive (delete-a-GAR-package), but the recipe/instance vocabulary split keeps them as parallel verbs — distinct name, colophon, spec, and surface vocabulary, with shared backbone living in registry-utility code. The Vessel:Hallmark :: Lode:{INSTANCE_NOUN} analogy carries through verbs; it does not collapse at the substrate. A future session tempted to unify because "the code is identical" should resist — the parallelism is load-bearing for the recipe model, even where the implementation is not. The principle extends to other cross-side registry operations.
...
- **Captured-side delete verb** (parallel to built-side `abjure`) is open. Forge/Solomonic register, lexically distinct from `abjure`, must parse against the prospective {INSTANCE_NOUN}. Colophon lands in the depot family `rbw-d?`. Implementation backbone shared with `abjure` via a registry-utility primitive; the verb itself stays distinct.
...
- ₣Az `rbk-90-deferred-ark-concept-debt` (retired 260513) — closed by GAR-package substrate finding; "ark" survives as the built-side grouping noun mapping to GAR `package`
