...
- [x] **Role Definitions** (manual)
  Added axo_identity and axo_actor to AXL. rbtr_role voices axo_identity (the abstract concept). Specific actors (rbtr_payor, rbtr_governor, rbtr_retriever, rbtr_mason, rbtr_director) voice axo_actor. Confirmed that guide-ness lives on operations, not actors - payor has GUIDE operations in its domain but the actor itself doesn't carry "human required" typing.
...
- Type model clarification: actors don't carry "human required" property; guide-ness is on operations in the actor's domain
