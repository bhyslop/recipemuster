...
An
{rbtgi_rbra_file}
is a credentials file containing the minimum identity and signing material needed for
{at_recipe_bottle}
...
{rbra_token_lifetime_sec}::
The OAuth token lifetime in seconds for JWT token exchange with
{gcp_service}
APIs.
Validated in range 300-3600; typically set to 1800 (30 minutes).
