...
and the *Concurrent Build CPUs* quota support the desired level of parallel build execution.
...
The primary control for concurrent build capacity is selecting a machine type whose vCPU count divides evenly into the available CPU quota:
...
.. Machine type is the primary lever to control concurrent capacity.
Reduce the machine type in the regime configuration to fit more builds in existing quota:
... Set
{rbrr_gcb_machine_type}
