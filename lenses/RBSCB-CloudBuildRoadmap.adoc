...
**Approach: per-vessel mini-repos with fully-baked yaml.**
A generator (adapted from `zrbf_stitch_build_json`) reads rbgjb/*.sh scripts plus current regime pins
and emits a static `cloudbuild.yaml` per vessel with ALL values baked — zero custom substitution overrides.
The master copy is committed to the main repo. The inscribe command syncs it to a per-vessel mini-repo
on GitHub that Cloud Build sees via Developer Connect.

**Per-vessel mini-repos (security boundary).**
Each vessel has a small GitHub repo containing only cloudbuild.yaml + Dockerfile + build context.
Cloud Build sees ONLY the mini-repo via Developer Connect — the main repo is never exposed to Google.
SLSA provenance records the mini-repo commit, which maps directly to the build material.
Mini-repo management is fully orchestrated by inscribe (clone/sync/commit/push).

**Everything is baked into the yaml (static, committed):**
...
Maximum SLSA purity: the build is fully determined by the committed yaml at a specific commit.
...
One yaml per vessel is the natural unit.

**Consequences:**

* GCS source tarball upload is eliminated — triggers fetch from the mini-repo via Developer Connect
* The stitch function (`zrbf_stitch_build_json`) is adapted to emit YAML instead of JSON; step scripts (rbgjb/*.sh) remain as source of truth, consumed at inscribe time
...
* Build context scope narrows: trigger checks out only the mini-repo (Dockerfile + context + yaml), not the entire project. Cleaner than the old tarball approach.

**Prerequisites:**

* Developer Connect setup (one-time interactive browser OAuth) — see RBSGD-gdc_establish.adoc
* Per-vessel mini-repo created on GitHub (inscribe creates if absent)
...
so converting to static cloudbuild.yaml per vessel is refactoring, not redesign.
(2026-02)

**Tier 1 log hygiene folded into Tier 2.**
Build step scripts are being inlined into cloudbuild.yaml during trigger migration.
Log hygiene (ensuring tokens and credentials are not echoed) is audited naturally during this inlining —
each rbgjb script is reviewed as its content moves into YAML step blocks.
...
**Main repo triggers over per-vessel repos.**
...
and per-vessel pin-refresh integration — substantial infrastructure for alpha. With main repo triggers:
Developer Connect uses an OAuth GitHub App (one-time browser auth, no PAT lifecycle), `triggers.run` dispatches
against the main repo commit, and Google still generates SLSA v1.0 provenance recording the exact commit hash.
The provenance commit may include non-vessel changes, but for a single-author alpha project this is acceptable.
