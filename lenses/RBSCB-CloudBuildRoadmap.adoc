...
**Approach: single rubric repo with fully-baked JSON.**
A generator (adapted from `zrbf_stitch_build_json`) reads rbgjb/*.sh scripts plus current regime pins
and emits a static `cloudbuild.json` per vessel with ALL values baked — zero custom substitution overrides.
The master copy is committed to the main repo. The inscribe command syncs it to a per-vessel directory
in the shared rubric repo that Cloud Build sees via Developer Connect.

**Single rubric repo (security boundary).**
One shared rubric repo on GitHub contains per-vessel directories, each holding cloudbuild.json + Dockerfile + build context.
Cloud Build sees ONLY the rubric repo via Developer Connect — the main repo is never exposed to Google.
SLSA provenance records the rubric repo commit, which maps directly to the build material.
Rubric repo management is fully orchestrated by inscribe (clone/sync/commit/push).

**Everything is baked into the JSON (static, committed):**

* Step structure, script bodies, step IDs, entrypoints
* Builder image digest references (digest-pinned from RBRR_GCB_*_IMAGE_REF)
* GAR coordinates, vessel identity, platforms, Dockerfile path, ark suffixes
* Inscribe timestamp in tag template (e.g., `i20260223.143022`)
* Per-build uniqueness via Cloud Build built-in `$BUILD_ID` (not a custom substitution)

**Zero custom substitution overrides at dispatch time.**
Maximum SLSA purity: the build is fully determined by the committed JSON at a specific commit.
The Foundry invokes `triggers.run` specifying only which commit to build — no `_RBGY_*` overrides.

**One JSON per vessel.**
Different vessels have different Dockerfiles, build contexts, and platforms.
One `cloudbuild.json` per vessel is the natural unit.

**Consequences:**

* GCS source tarball upload is eliminated — triggers fetch from the rubric repo via Developer Connect
* The stitch function (`zrbf_stitch_build_json`) is adapted to emit the full trigger-compatible JSON structure; step scripts (rbgjb/*.sh) remain as source of truth, consumed at inscribe time
* Pin updates are per-vessel: operator runs inscribe for a specific vessel after refreshing pins
* Google SLSA provenance is generated automatically — no DIY provenance needed
* Build context scope narrows: trigger checks out the rubric repo (per-vessel directories with Dockerfile + context + JSON), not the entire project. Cleaner than the old tarball approach.

**Prerequisites:**

* Developer Connect setup (one-time interactive browser OAuth) — see RBSGD-gdc_establish.adoc
* Rubric repo created on GitHub with per-vessel directories (inscribe creates if absent)
* Cloud Build trigger resource created per vessel (inscribe creates if absent)
...
* Add `options.pool.name` to the build request (or `cloudbuild.json` if Tier 2 is complete)
...
Converting to a static `cloudbuild.json` is mechanical, not architectural.
...
The generated-artifact-must-be-committed pattern preserves the Foundry's generation logic while producing a trigger-compatible JSON.
One `cloudbuild.json` per vessel chosen over a shared config for clarity.
(2026-02)
...
Google automatically generates SLSA v1.0 provenance for trigger-invoked builds.
This is the single highest-value security improvement available before alpha release.
The mechanical cost is justified: the Foundry already parameterizes builds through substitution variables,
so converting to static cloudbuild.json per vessel is refactoring, not redesign.
(2026-02)

**Tier 1 log hygiene folded into Tier 2.**
Build step scripts are being inlined into cloudbuild.json during trigger migration.
Log hygiene (ensuring tokens and credentials are not echoed) is audited naturally during this inlining —
each rbgjb script is reviewed as its content moves into JSON step blocks.
A separate Tier 1 log hygiene effort would duplicate this audit.
(2026-02)

**BuildKit --provenance flag deferred.**
Google trigger-level SLSA v1.0 provenance (Tier 2) covers the primary use case:
attesting that a specific build produced a specific image from a specific source commit.
BuildKit-level provenance (`--provenance=true` on `docker buildx build`) records builder internals —
useful for deeper forensics but redundant for the current threat model.
Revisit if build reproducibility verification becomes a priority.
(2026-02)

**Tiers 3-5 deferred to post-MVP.**
Private Pools (Tier 3), Egress Lockdown (Tier 4), and VPC Service Controls (Tier 5) represent
progressive GCB infrastructure investment with increasing lock-in and maintenance cost.
None provide security properties required for an alpha release consumed by the project author.
Each tier remains a separate future heat decision, ordered by the dependency chain documented above.
(2026-02)

**Dockerfile base image digest validation — independent, future heat.**
Validating that vessel Dockerfiles use digest-pinned `FROM` references is a supply-chain hygiene measure
orthogonal to the trigger migration. It constrains vessel authors (policy decision with RBSRV implications)
and does not depend on Tier 2 infrastructure. Tracked as a separate future heat, not a Tier 2 prerequisite.
(2026-02)

**Single rubric repo over per-vessel repos.**
The original Tier 2 design used per-vessel GitHub repos for provenance isolation (one commit = one vessel's
build definition). This required a Courier PAT regime, N repo creation/management scripts, a publish ceremony,
and per-vessel pin-refresh integration — substantial infrastructure for alpha. A single shared rubric repo
with per-vessel directories provides the same security boundary (Cloud Build sees only the rubric repo, not
the main repo) with one repo to manage instead of N. Developer Connect uses an OAuth GitHub App (one-time
browser auth, no PAT lifecycle), `triggers.run` dispatches against the rubric repo commit, and Google still
generates SLSA v1.0 provenance recording the exact commit hash. Per-vessel repos remain a future option if
stricter provenance isolation becomes a real requirement.
(2026-02)
