...
//axhos_step
. *Delete Private Worker Pool*:
.. Construct pool resource name: `projects/«DEPOT_PROJECT_ID»/locations/{rbrr_gcp_region}/workerPools/rbw-«INPUT_DEPOT_NAME»-pool`
.. {rbbc_call}
...
.. {rbbc_require}
HTTP 200 or 404 response; any other status is {rbbc_fatal}
...
The private worker pool is explicitly deleted before project deletion because GCP project deletion does not automatically cascade to worker pool resources — an undrained pool can block or delay the delete operation.
