...
include::rbw-RBSGS-GettingStarted.adoc[leveloffset=+2]
...
include::rbw-RBSPE-payor_establish.adoc[]
...
include::rbw-RBSPI-payor_install.adoc[]

[[rbtgo_payor_refresh]]
// ⟦axl_voices axo_guide axe_human_guide⟧
=== {rbtgo_payor_refresh}

include::rbw-RBSPR-payor_refresh.adoc[]
...
include::rbw-RBSDD-depot_destroy.adoc[]

[[rbtgo_depot_list]]
// ⟦axl_voices axo_command axe_bash_interactive⟧
=== {rbtgo_depot_list}

include::rbw-RBSDL-depot_list.adoc[]

[[rbtgo_governor_reset]]
// ⟦axl_voices axo_command axe_bash_interactive⟧
=== {rbtgo_governor_reset}

include::rbw-RBSGR-governor_reset.adoc[]
...
include::rbw-RBSRC-retriever_create.adoc[]

[[rbtgo_director_create]]
// ⟦axl_voices axo_command axe_bash_interactive⟧
=== {rbtgo_director_create}

include::rbw-RBSDI-director_create.adoc[]
...
NOTE: For multi-platform build authentication challenges and research findings, see link:rbw-RBWMBX-BuildxMultiPlatformAuth.adoc[RBWMBX: Buildx Multi-Platform Authentication Memo].

[[rbtgo_image_delete]]
// ⟦axl_voices axo_command axe_bash_interactive⟧
=== {rbtgo_image_delete}

include::rbw-RBSID-image_delete.adoc[]
...
include::rbw-RBSIR-image_retrieve.adoc[]
...
include::rbw-RBSSL-sa_list.adoc[]

[[rbtgo_sa_delete]]
// ⟦axl_voices axo_command axe_bash_interactive⟧
=== {rbtgo_sa_delete}

include::rbw-RBSSD-sa_delete.adoc[]
...
Push Phase (has credentials via metadata server):
  skopeo copy --all --dest-registry-token "${TOKEN}" \
...
* **Separation of concerns**: BuildKit builds (what it does best), Skopeo pushes (what it does best)
* **No credential propagation needed**: Build phase writes to local filesystem, push phase uses
{gcb_service}
metadata server
* **Google-endorsed**: Skopeo explicitly recommended in https://cloud.google.com/blog/topics/developers-practitioners/five-ways-skopeo-can-simplify-your-google-cloud-container-workflow[Google Cloud blog]
* **Token timeout eliminated**: Build phase can take hours; push phase takes seconds with fresh
{oauth_token}
...
**Push step** (rbgjb07-push-with-skopeo.sh):
----
...
* Use `--dest-registry-token` not `--dest-creds` for OAuth bearer
{oauth_token_s}
...
* Skopeo's `oci-archive:` transport reads tarballs directly
* Syft cannot analyze multi-platform OCI archives directly; must pull image first and analyze via docker socket

==== References

* Research memo: link:rbw-RBWMBX-BuildxMultiPlatformAuth.adoc[RBWMBX: Buildx Multi-Platform Authentication Memo]
