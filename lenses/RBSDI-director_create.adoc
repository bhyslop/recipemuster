...
//axhoq_precondition
* Execute {rbtoe_check_devconnect} to verify {rbtgi_source_connection} authorization
...
.. NOTE: Uses numeric ID URL — immediately resolvable, no propagation delay.
.. {rbbc_require}
...
.. NOTE: All member bindings use `serviceAccount:«DIRECTOR_UNIQUE_ID»` (numeric ID) to avoid GAIA propagation race.
.. {rbbc_call}
link:https://cloud.google.com/resource-manager/reference/rest/v3/projects/setIamPolicy[projects.setIamPolicy] for
{rbrr_depot_project_id}
.. Add `serviceAccount:«DIRECTOR_UNIQUE_ID»` with roles:
... `roles/cloudbuild.builds.editor` (submit and manage builds)
... `roles/viewer` (read project resources)
... `roles/developerconnect.admin` (source link and trigger infrastructure)
.. Policy version: 3
.. {rbbc_require}
...
.. NOTE: Uses email, not numeric ID — Mason is the IAM resource target, not the newly-created member.
.. {rbbc_require}
...
{rbtgi_repository}
.. Add `serviceAccount:«DIRECTOR_UNIQUE_ID»` with `roles/artifactregistry.repoAdmin`
.. {rbbc_require}
