...
.. Add `serviceAccount:«DIRECTOR_EMAIL»` to `roles/secretmanager.secretAccessor` in the existing policy (read-modify-write)
.. {rbbc_call}
...
.. {rbbc_require}
IAM binding successful — existing bindings preserved
...
{rbtgi_repository}
.. Add `serviceAccount:«DIRECTOR_EMAIL»` with `roles/artifactregistry.repoAdmin`
.. {rbbc_require}
repository IAM update successful
