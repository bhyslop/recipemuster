...
# Step 7: Push OCI archive with Skopeo (has authentication)
...
NOTE: Use `--dest-registry-token` (not `--dest-creds`) per https://cloud.google.com/blog/topics/developers-practitioners/five-ways-skopeo-can-simplify-your-google-cloud-container-workflow[Google Cloud blog]. The `oci-archive:` transport reads tarballs; `oci:` transport reads directories.
...
**Step 6: Push with Skopeo** (rbgjb07-push-with-skopeo.sh):
----
skopeo copy --all --dest-registry-token "${AR_TOKEN}" \
  "oci-archive:/workspace/oci-layout.tar" \
  "docker://${IMAGE_URI}"
----
...
Initial research suggested using `tar=false` for directory output compatible with Skopeo's `oci:` transport. However, testing revealed issues with `tar=false`:
...
**Proven solution**: Use default tarball output with Skopeo's `oci-archive:` transport:

----
# Build step: tarball output (default, no tar= flag needed)
--output type=oci,dest=/workspace/oci-layout.tar

# Push step: oci-archive transport reads tarballs
skopeo copy --all "oci-archive:/workspace/oci-layout.tar" "docker://..."
----

The `oci-archive:` transport reads OCI tarballs directly. No extraction needed.
...
Use `--dest-registry-token` NOT `--dest-creds`:

- `--dest-creds="oauth2accesstoken:${TOKEN}"` — returns 403 Forbidden
- `--dest-registry-token "${TOKEN}"` — works correctly

The Google Cloud blog example uses `--dest-registry-token`. This is the correct approach for OAuth bearer tokens.

=== IAM Permissions

The Mason service account requires `roles/artifactregistry.admin` on the GAR repository resource (not just project-level). Without this, Skopeo push fails with permission denied.
