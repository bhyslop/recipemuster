...
.. For each service in [`artifactregistry`, `cloudbuild`, `containeranalysis`, `developerconnect`, `iam`, `secretmanager`, `serviceusage`, `storage`]:
... Execute
{rbtoe_api_enable}
...
.. {rbbc_store}
`«MASON_UNIQUE_ID»` from response `.uniqueId` field

//axhos_step
. *Configure
{rbtr_mason}
Permissions*:
.. NOTE: All member bindings use `serviceAccount:«MASON_UNIQUE_ID»` (numeric ID) to avoid GAIA propagation race.
.. {rbbc_call}
...
... Add `serviceAccount:«MASON_UNIQUE_ID»` with `roles/artifactregistry.writer`
.. {rbbc_call}
link:https://cloud.google.com/storage/docs/json_api/v1/buckets/setIamPolicy[buckets.setIamPolicy] for `«BUILD_BUCKET»`
... Add `serviceAccount:«MASON_UNIQUE_ID»` with `roles/storage.objectViewer`
.. {rbbc_call}
...
... Add `serviceAccount:«MASON_UNIQUE_ID»` with `roles/viewer`
... Add `serviceAccount:«MASON_UNIQUE_ID»` with `roles/logging.logWriter`
...
.. NOTE: Uses email, not numeric ID — Cloud Build service agent is Google-provisioned, not newly created.
.. {rbbc_require}
IAM binding successful (enables Cloud Build to execute as Mason)

//axhos_step
. *Create Developer Connect Connection*:
.. NOTE: The connection is created in PENDING state. The
{at_operator}
completes GitHub OAuth authorization later via
{rbtgo_depot_initialize}.
.. Construct connection name: `rbw-«INPUT_DEPOT_NAME»-github`
.. {rbbc_call}
link:https://cloud.google.com/developer-connect/docs/reference/rest/v1/projects.locations.connections/create[developerconnect.connections.create] with:
... parent: `projects/«DEPOT_PROJECT_ID»/locations/«INPUT_REGION»`
... connectionId: `rbw-«INPUT_DEPOT_NAME»-github`
... body: `{"githubConfig": {"githubApp": "DEVELOPER_CONNECT"}}`
.. {rbbc_await}
returned {gcp_lro}
.. {rbbc_store}
`«GDC_CONNECTION_NAME»` as `rbw-«INPUT_DEPOT_NAME»-github`
.. {rbbc_require}
connection exists (HTTP 200 on GET) with `installationState.stage` of `PENDING_CREATE_APP` or similar pending state

//axhos_step
. *Grant Developer Connect Service Agent Secret Manager Access*:
.. NOTE: The Developer Connect service agent needs Secret Manager permissions to store the GitHub OAuth token during
{rbtgo_depot_initialize}.
.. Construct service agent: `service-«PROJECT_NUMBER»@gcp-sa-devconnect.iam.gserviceaccount.com`
.. {rbbc_call}
link:https://cloud.google.com/resource-manager/reference/rest/v3/projects/setIamPolicy[projects.setIamPolicy] for `«DEPOT_PROJECT_ID»`
.. Add Developer Connect service agent with `roles/secretmanager.admin`
.. {rbbc_require}
...
... `RBRR_GDC_CONNECTION_NAME=«GDC_CONNECTION_NAME»`
... `RBRR_GDC_REGION=«INPUT_REGION»`
.. {rbbc_show}
Mason service account: `«MASON_EMAIL»`
.. {rbbc_show}
Developer Connect connection `«GDC_CONNECTION_NAME»` created in PENDING state
.. {rbbc_show}
depot ready for
{rbtgo_depot_initialize}
to complete GitHub OAuth authorization
...
//axhoc_completion
Successful execution creates depot infrastructure with configured build bucket, container repository, Mason service account, and Developer Connect connection (PENDING state).
Proceed to
