...
The
{rbtgi_repository}
within the depot stores all
{rbtgi_image_s}
available for retrieval.
Most organizations require exactly one depot.

{at_recipe_bottle}
provides bash command-line administration of depot lifecycle and operations across roles.

* *{rbtr_payor}*: Creates and funds depots; creates governors. Billing flows through the payor's linked account. Manual OAuth setup enables all subsequent automation.
* *{rbtr_governor}*: Administers credentials within a depot. Creates directors and retrievers via service account authentication.
* *{rbtr_director}*: Submits builds, manages images.
* *{rbtr_retriever}*: Pulls images for local use.

The
{rbtr_payor}
stands apart--requires manual console work and OAuth authentication for both depot and governor lifecycle operations.
The
{rbtr_governor}
...
The
{rbtr_payor}
can manage multiple depots:
{rbtgo_depot_list}
displays all active depots and their status;
{rbtgo_depot_destroy}
permanently removes a depot and all its resources.
...
{rbtgi_depot}.
This Payor operation produces the first
{rbtgi_rbra_file},
