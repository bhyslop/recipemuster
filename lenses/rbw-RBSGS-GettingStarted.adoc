...
Welcome.
{at_recipe_bottle}
solves a specific problem: you build
{at_bottle_image_s}
on your laptop, your teammate builds the same code on theirs, and they're *identical*.
...
{gcp_organization}
and the computers where people build.
...
Google hands you a
{rbro_refresh_token}
- a long-lived credential that lets Recipe Bottle stay logged in without asking you every time.
...
* The
{rbtgi_json_key_file}
(keep it safe, you might need it if things go wrong)
...
creates your first
{rbtgi_depot}
- the project and all its infrastructure where containers actually get built in the cloud.
...
**Why you're doing this:** You're creating the actual workshop where your containers are safely constructed.
...
* `RBRO_CLIENT_SECRET` - your
{rbro_client_secret}
* `RBRO_REFRESH_TOKEN` - your
{rbro_refresh_token}
...
* If you lose it, you have to redo the
{rbtr_payor}
installation (annoying, but not catastrophic)
...
This is the same care you'd take with your house keys.
We designed Recipe Bottle to handle this file with due respect, so you can trust it.

**2. The OAuth
{rbtgi_json_key_file}
** (your downloaded `client_secret_*.json`)
This file contains the same secret.
Once you've run
{rbtgo_payor_install},
keep it safe but you can stop thinking about it.
...
for the
{rbtr_director}
role lets them submit builds.
A different
{rbtgi_rbra_file}
for the
{rbtr_governor}
role lets them manage projects.
...
Her
{rbtgi_image_s}
is bit-for-bit identical to yours.
...
The setup you did - all of it, the
{rbtr_payor},
the
{rbtgi_depot},
all the credential files - protects you from those questions.
...
Create a new
{rbtgi_rbra_file}
for them with just the role they need.
...
Once your
{rbtgi_depot}
is running, you'll interact with Recipe Bottle through different roles:

* {rbtr_director}: You submit builds, manage
{rbtgi_image_s}
* {rbtr_governor}: You manage the depot project itself
* {rbtr_retriever}: You (or your team) pull
{rbtgi_image_s}
to use them
