...
* *{rbtr_payor}*: Creates and funds depots. Billing flows through the payor's linked account. Manual OAuth setup enables all subsequent automation.
* *{rbtr_governor}*: Administers credentials within a depot. Creates directors and retrievers.
* *{rbtr_director}*: Submits builds, manages images.
* *{rbtr_retriever}*: Pulls images for local use.

The
{rbtr_payor}
stands apart--requires manual console work and OAuth authentication.
All other roles authenticate via
{rbtgi_rbra_file}
