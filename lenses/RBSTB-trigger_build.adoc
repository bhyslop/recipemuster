...
Multiple sessions can invoke this command simultaneously against the same repository without collision.
Each invocation uses isolated temporary directories and uniquely-named cloud artifacts.
...
.. Create gzip tarball from staging directory (source only; build steps inlined in API request)
.. {rbbc_store}
...
.. {gcb_service}
executes the following phases within `«DEPOT_PROJECT_ID»`:
