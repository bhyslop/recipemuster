...
.. {rbbc_require}
`«INPUT_ARK_STAMP»` provided as second argument
.. {rbbc_fatal}
if moniker missing: "Moniker parameter required"
.. {rbbc_fatal}
if ark stamp missing: "Ark stamp parameter required"

. *Validate Ark Stamp Format*:
.. {rbbc_require}
`«INPUT_ARK_STAMP»` matches pattern `YYYYMMDDTHHMMSSZ` or similar timestamp format
.. {rbbc_fatal}
if malformed: "Ark stamp format invalid"
...
. *Construct Ark Tags*:
.. {rbbc_store}
`«IMAGE_TAG»` as `«INPUT_MONIKER»:«INPUT_ARK_STAMP»-image`
.. {rbbc_store}
`«ABOUT_TAG»` as `«INPUT_MONIKER»:«INPUT_ARK_STAMP»-about`
...
... method: `HEAD`
... endpoint: `«REGISTRY_BASE»/«INPUT_MONIKER»/manifests/«INPUT_ARK_STAMP»-image`
...
... endpoint: `«REGISTRY_BASE»/«INPUT_MONIKER»/manifests/«INPUT_ARK_STAMP»-about`
.. {rbbc_store}
...
... {rbbc_show}
"Will abjure ark «INPUT_MONIKER»/«INPUT_ARK_STAMP»:"
... For each artifact in `«ARTIFACTS_TO_DELETE»`:
.... {rbbc_show}
`  - «TAG»`
... {rbbc_prompt}
"Proceed with abjuration? [y/N]"
... {rbbc_require}
user confirmation
... {rbbc_fatal}
if declined: "Abjuration cancelled"
...
. *Display Results*:
.. {rbbc_show}
"Ark abjured: «INPUT_MONIKER»/«INPUT_ARK_STAMP»"
