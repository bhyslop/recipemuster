...
> **POST-IMPLEMENTATION UPDATE (2026-05-12):** Round 3's reasoning that "source-CIDR is structurally broken on bridge-gateway-source-rewrite paths" was empirically wrong on Docker Desktop. The actual Desktop source IP is `192.168.65.1` (Desktop's host-to-VM gateway), outside any enclave CIDR. Source-CIDR exclusion is the correct and load-bearing predicate enforcing the `net-dnat-entry-reflection` invariant. The empirically-validated fix shape (macOS three-fixture parallel green) combines Round 3's source-CIDR (correct) + POSTROUTING MASQUERADE (Round 3 missed) + conntrack-DNAT FORWARD ACCEPT (Round 3 had) + ESTABLISHED,RELATED baseline (already present) + `rp_filter=2` loose (neither this section nor any prior round identified — discovered via scry-empirical diagnosis). See `Memos/memo-20260512-sentry-entry-port-repair-anchor.md` for the empirically-corrected fix recipe with full per-clause rationale; the content below is retained as historical record of Round 3's framing.

**Round 3 simplification (historical, partially refuted).** A third round of research established that source-CIDR exclusion at PREROUTING is structurally broken in the bridge-gateway-source-rewrite case (see the new row in the Documented vs Observed table above), not merely fragile. The Round 2 fix proposed source-CIDR exclusion as the classification predicate at PREROUTING; that predicate cannot work when the source IP of hairpinned published-port traffic is the enclave bridge gateway, because the gateway lies inside any enclave CIDR by topological necessity. Renumbering produces a new gateway at `.1` of the new CIDR. The classification predicate must change.
...
> **POST-IMPLEMENTATION UPDATE (2026-05-12):** This reframe was implemented and **empirically refuted** on macOS Docker Desktop 28.x. A `tt/rbw-cs.Scry.sh` packet capture during the failed iteration showed sentry's RBM-FORWARD counters at zero — the bottle's SYN-ACK never traversed sentry's iptables FORWARD chain — and Docker Desktop's networking layer reflected the SNAT'd return through sentry's transit interface hostilely. Root cause: the bottle's default-via-sentry routing makes inbound published-port traffic produce asymmetric return paths, fundamentally incompatible with our egress-gate-via-default-route security model. The pre-regression design's POSTROUTING MASQUERADE on entry-port traffic was structurally load-bearing (not decorative) because it kept the bottle's reply destination in-enclave; the topology reframe broke this invariant. See `Memos/memo-20260512-sentry-entry-port-repair-anchor.md` for the empirical evidence in full and the canonical fix path that supersedes this section. The reframe content below is retained as historical record of the hypothesis.
