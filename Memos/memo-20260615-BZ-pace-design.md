...
Recovered intent. Movement 6 is the federation narrative-docs movement: the README story plus broadside registration of the elected federation words (the README glossary is the project broadside), post-build cold-probe verifying first-contact actionability. The contract specs rode their M4 paces and the RBS0 civic quoins seated at the verb movement's head, so neither is M6; the handbook rework is routed to ₣A6 (the slate-picks above). M6 is therefore README + broadside glossary + diagrams only.

The as-built inversion (why M6 exceeds its one-liner). The README still frames keyfile as today's whole system and federation as a planned future tier ("the planned Operator Federation tier," "the path to corporate-acceptable identity," the keyfile-login diagram captioned "today's tier, for contrast"). M6 inverts this: federation becomes the model, keyfile becomes retired bridge legacy and — post-demolition — simply gone, the two-tier framing collapsing to one. Three sharper findings beyond the inversion: (1) the broadside already carries the mantle homonym — the glossary defines "Mantle — Create or replace the Governor service account," the cult/keyfile verb sense (the one renamed enrobe at M4), which must flip to the federation standing-role noun, meaning and part of speech both; (2) the federation-seam diagram caption "one accessor, mode-enum branch" is stale — this heat canceled the mode enum (single tier); (3) the RBRA glossary entry and the keyfile-login diagram (rbdgk) retire with the estate, while RBRO (the payor's durable OAuth secret) stays.
...
- Orphaned SA-key retry constants (left standing by the ₢BZAAe demolition, gated on the RBSCIG/RBSCIP reconciliation above): the keyfile JWT mint's removal orphaned `RBGC_SA_KEY_CONSUMER_RETRY_*` (BUDGET/INITIAL_DELAY/MAX_DELAY) in rbgc_constants.sh — now zero bash consumers; `RBGC_SA_KEY_CREATE_RETRY_*` (MAX/DELAY) was likewise orphaned earlier by the enrobe key-creation removal. Both are dead code, NOT removed by ₢BZAAe because the only remaining references are the spec citations being reanchored in the two bullets above — RBSCIP:53 / RBSCIP:205 cite `RBGC_SA_KEY_*_RETRY_*`, RBSCIG:350-351 cite `RBGC_SA_KEY_CREATE_RETRY_{MAX,DELAY_SEC}`. Removing the constants ahead of Fable would dangle those citations; Fable removes the five constants + reconciles the prose in one pass (delete the consts when the surviving flap physics re-anchor to mantle-SA levy/unmake lifecycle, which mint no per-call keys and need no consumer-side propagation retry).
