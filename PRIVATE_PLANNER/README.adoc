...
I am an embedded developer who wants to use more containers but I feel I need some additional security measures to protect my and my customer assets.
To this end, I have undertaken as a passion project https://scaleinv.github.io/recipebottle/ with full open source intent.
At that link, you will find an abstract of the project.

TL;DR perhaps.

In short, I want a container running less trusted code ("BOTTLE") to be able to use a container running a trusted, carefully configured intermediary container ("SENTRY") as its gateway.
When I hand construct the BOTTLE dockerfile, I can compel startup machinations that make this work.
I'd really prefer to have a means of using unadulterated dockerfiles/ images for BOTTLE, though.
Could there be a simple way to run it such that it isn't even aware it is in a BOTTLE?
...
A search of your open issues led me to https://github.com/containers/podman/issues/20222 which may be consonant with this feature request.
I'm afraid I'm not deep enough into networking to determine this on my own.

==== Alternatives Considered

My road to this feature request has been quite long.
Since I'm not a deep networking expert, I've been freely using Claude from Anthropic to help me navigate, and eventually ChatGPT too.
...
This was long enough ago that it may not be true anymore, but it was enough to lead me to try `podman`.
...
This was enough to prove the SENTRY/ BOTTLE concept, and to justify the 'container image lifecycle management playground' engineering I've already built out for the project.
...
I tried a bevvy of similar approaches that led me to explore `--opt` and `--dns` options in `podman run` and `podman network` commands: but between me and my AI, I couldn't break through with these: they feel some combination of power tools for the network wise and features that are falling out of support.
...
I then played some with setting up `dhclient` in the BOTTLE to effectively post-configure the network stack, but this failed probably because of 'combativeness' between my networking and `podman`'s own.
I additionally didn't love this for same reasons as the previous approach: intimate BOTTLE manipulation is not great.
...
I went far down the rabbit hole of exploring arp cache machenations and `podman machine` network namespace tracing (my laptop is windows), but ultimately that felt fragile.
I want this project to work well inside and outside of a `podman machine` after all.

ChatGPT had some ideas about using CNI to set up a custom gateway container- I'd flirted with some CNI settings under Claude's suggestions, without a lot of traction.
...
Other runtimes with more kubernetes ties seem sophisticated and complex, without having the developer workstation focus of `podman`.
