...
read -r z_num RBJS_ENCLAVE_IF z_rest < "${z_temp_file}"
test -n "${RBJS_ENCLAVE_IF}" || { echo "FATAL: No interface found with IP ${RBRN_ENCLAVE_SENTRY_IP}"; exit 11; }

ip -o -4 addr show scope global > "${z_temp_file}" || exit 11
RBJS_UPLINK_IF=""
while read -r z_num z_ifname z_rest; do
  test "${z_ifname}" = "${RBJS_ENCLAVE_IF}" && continue
  RBJS_UPLINK_IF="${z_ifname}"
  break
done < "${z_temp_file}"
test -n "${RBJS_UPLINK_IF}" || { echo "FATAL: No uplink interface found"; exit 11; }

rm -f "${z_temp_file}"
echo "RBJp1: Enclave interface = ${RBJS_ENCLAVE_IF}"
echo "RBJp1: Uplink interface  = ${RBJS_UPLINK_IF}"

echo "RBJp1: Computing uplink gateway from connected subnet"
z_uplink_addr_cidr=$(ip -o -4 addr show dev "${RBJS_UPLINK_IF}" | awk '{print $4; exit}')
test -n "${z_uplink_addr_cidr}" || { echo "FATAL: No IPv4 address on uplink interface ${RBJS_UPLINK_IF}"; exit 12; }
z_uplink_ip="${z_uplink_addr_cidr%/*}"
z_uplink_prefix="${z_uplink_addr_cidr#*/}"

z_o1="${z_uplink_ip%%.*}"
z_octet_rest="${z_uplink_ip#*.}"
z_o2="${z_octet_rest%%.*}"
z_octet_rest="${z_octet_rest#*.}"
z_o3="${z_octet_rest%%.*}"
z_o4="${z_octet_rest#*.}"
...
RBJS_UPLINK_GW="$(( (z_gw_int >> 24) & 0xFF )).$(( (z_gw_int >> 16) & 0xFF )).$(( (z_gw_int >> 8) & 0xFF )).$(( z_gw_int & 0xFF ))"
echo "RBJp1: Uplink gateway     = ${RBJS_UPLINK_GW}"

echo "RBJp1: RBr_2c9: Replacing default route via uplink (sentry-side ownership)"
ip route replace default via "${RBJS_UPLINK_GW}" dev "${RBJS_UPLINK_IF}" || exit 13
...
iptables -A RBM-INGRESS -i "${RBJS_ENCLAVE_IF}" -p icmp -j ACCEPT || exit 20
iptables -A RBM-EGRESS  -o "${RBJS_ENCLAVE_IF}" -p icmp -j ACCEPT || exit 20
...
if test "${RBRN_ENTRY_MODE}" = "rbnne_enabled"; then
  ...
  iptables -t nat -A POSTROUTING -o "${RBJS_ENCLAVE_IF}" -p tcp \
...
iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -p icmp -j DROP || exit 28
...
if test "${RBRN_UPLINK_ACCESS_MODE}" = "rbnne_disabled"; then
  ...
  iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -j DROP || exit 30
  iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -j DROP || exit 30
else
  echo "RBJp3: Setting up uplink network hardening"
  echo 0 > /proc/sys/net/ipv6/conf/all/disable_ipv6    || exit 31
  echo 1 > "/proc/sys/net/ipv4/conf/${RBJS_UPLINK_IF}/route_localnet" || exit 31

  echo "RBJp3: Configuring NAT"
  iptables -t nat -A POSTROUTING -o "${RBJS_UPLINK_IF}" -s "${RBRN_ENCLAVE_BASE_IP}/${RBRN_ENCLAVE_NETMASK}" \
  ...
  if test "${RBRN_UPLINK_ACCESS_MODE}" = "rbnne_global"; then
    ...
    iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -j ACCEPT || exit 31
    iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -j ACCEPT || exit 31
  else
    echo "RBJp3: Configuring DNS server access"
    iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -p udp --dport 53 -d "${RBRR_DNS_SERVER}"        -j ACCEPT || exit 31
    iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -p tcp --dport 53 -d "${RBRR_DNS_SERVER}"        -j ACCEPT || exit 31
    iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -p udp --dport 53 -d "${RBRN_ENCLAVE_SENTRY_IP}" -j ACCEPT || exit 31
    iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -p tcp --dport 53 -d "${RBRN_ENCLAVE_SENTRY_IP}" -j ACCEPT || exit 31
    iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -p udp --dport 53                                -j DROP   || exit 31
    iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -p tcp --dport 53                                -j DROP   || exit 31

    echo "RBJp3: Setting up CIDR-based access control"
    for cidr in ${RBRN_UPLINK_ALLOWED_CIDRS}; do
      iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -d "${cidr}" -j ACCEPT || exit 32
      iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -d "${cidr}" -j ACCEPT || exit 32
    done
  fi
fi
...
if test "${RBRN_UPLINK_DNS_MODE}" = "rbnne_disabled"; then
  ...
  iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -p udp --dport 53 -j DROP || exit 40
  iptables -A RBM-FORWARD -i "${RBJS_ENCLAVE_IF}" -p tcp --dport 53 -j DROP || exit 40
  iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -p udp --dport 53 -j DROP || exit 40
  iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -p tcp --dport 53 -j DROP || exit 40
else
  ...
  echo "interface=${RBJS_ENCLAVE_IF}"                             >> /etc/dnsmasq.conf || exit 41
  echo "listen-address=${RBRN_ENCLAVE_SENTRY_IP}"               >> /etc/dnsmasq.conf || exit 41
  echo "no-dhcp-interface=${RBJS_ENCLAVE_IF}"                     >> /etc/dnsmasq.conf || exit 41
  ...
  echo "except-interface=${RBJS_UPLINK_IF}"                       >> /etc/dnsmasq.conf || exit 41
  ...
  iptables -A RBM-INGRESS -i "${RBJS_ENCLAVE_IF}" -p udp --dport 53                         -j ACCEPT || exit 43
  iptables -A RBM-INGRESS -i "${RBJS_ENCLAVE_IF}" -p tcp --dport 53                         -j ACCEPT || exit 43
  iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -p udp --dport 53 -d "${RBRR_DNS_SERVER}" -j ACCEPT || exit 43
  iptables -A RBM-EGRESS  -o "${RBJS_UPLINK_IF}" -p tcp --dport 53 -d "${RBRR_DNS_SERVER}" -j ACCEPT || exit 43
fi
