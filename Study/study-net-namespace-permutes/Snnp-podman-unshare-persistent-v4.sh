...
echo -e "${BOLD}Using podman unshare to create persistent network namespace (v4)${NC}"
echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Creating persistent namespace, configuring from host"

# Create a simple script that just keeps the namespace alive
snnp_machine_ssh "cat > /tmp/persistent_netns_v4.sh << 'EOF'
#!/bin/bash
set -e

echo 'Starting podman unshare namespace keeper'
echo 'Current PID: \$\$'
...
echo 'Namespace created, waiting for host to configure networking...'
...
echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Getting the unshare process PID"
UNSHARE_PID=$(snnp_machine_ssh "cat /tmp/unshare_pid_${NET_NAMESPACE}")
echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Unshare PID: ${UNSHARE_PID}"

echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Creating veth pair in host namespace"
snnp_machine_ssh_sudo ip link add ${ENCLAVE_BOTTLE_OUT} type veth peer name ${ENCLAVE_BOTTLE_IN}

echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Moving veth into the unshare namespace"
snnp_machine_ssh_sudo ip link set ${ENCLAVE_BOTTLE_IN} netns ${UNSHARE_PID}

echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Configuring interface in namespace using nsenter from host"
...
echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Setting default route in namespace"
snnp_machine_ssh_sudo nsenter -t ${UNSHARE_PID} -n ip route add default via ${ENCLAVE_SENTRY_IP}

echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Connecting veth to bridge from host namespace"
...
echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Verifying namespace setup"
snnp_machine_ssh "ls -la /proc/${UNSHARE_PID}/ns/"
snnp_machine_ssh "echo 'Process still running:' && ps -p ${UNSHARE_PID}"
snnp_machine_ssh_sudo nsenter -t ${UNSHARE_PID} -n ip addr show

echo "RBNS-PODMAN-UNSHARE-PERSISTENT-V4: Starting container with the prepared network namespace"
podman -c ${MACHINE} run -d                       \
...
echo "RBNI: Container namespace network info"
snnp_machine_ssh_sudo nsenter -t ${UNSHARE_PID} -n ip addr show
