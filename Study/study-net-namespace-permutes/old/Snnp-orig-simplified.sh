...
if [ -n "${BOTTLE_PID}" ] && [ "${BOTTLE_PID}" != "0" ]; then
  ...
  snnp_machine_ssh_sudo ip link set ${ENCLAVE_BOTTLE_IN} netns ${BOTTLE_PID}
  snnp_machine_ssh_sudo nsenter -t ${BOTTLE_PID} -n ip link set ${ENCLAVE_BOTTLE_IN} name eth1
  snnp_machine_ssh_sudo nsenter -t ${BOTTLE_PID} -n ip addr add ${ENCLAVE_BOTTLE_IP}/${ENCLAVE_NETMASK} dev eth1
  snnp_machine_ssh_sudo nsenter -t ${BOTTLE_PID} -n ip link set eth1 up
  ...
  snnp_machine_ssh_sudo nsenter -t ${BOTTLE_PID} -n ip route add default via ${ENCLAVE_SENTRY_IP}
  snnp_machine_ssh_sudo nsenter -t ${BOTTLE_PID} -n bash -c "echo 'nameserver ${ENCLAVE_SENTRY_IP}' > /etc/resolv.conf"
  ...
  snnp_machine_ssh podman run -d --name ${BOTTLE_CONTAINER} --network ns:/var/run/netns/${MONIKER}-ns --privileged ${BOTTLE_REPO_PATH}:${BOTTLE_IMAGE_TAG}
