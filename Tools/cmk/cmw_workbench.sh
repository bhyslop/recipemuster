...
zcmw_emit_prep_pr() {
  {
    ...
    echo ""
    echo "3. **Auto-detect next candidate branch:**"
    echo "   - Find max batch from upstream: \`git ls-remote --heads ${ZCMW_UPSTREAM_REMOTE} | grep 'candidate-'\`"
    echo "   - Find max batch from local branches"
    echo "   - If batch exists upstream → new batch (previous merged)"
    echo "   - If batch not upstream → redo (increment revision)"
    echo "   - Tell user which branch and why"
    echo ""
