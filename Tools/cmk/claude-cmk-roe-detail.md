...
- Lift the frame and these become two of **three edges** where the system meets the outside world, each with its own sovereign discipline: Interface Contamination guards the **input edge** (what we accept inward, in one canonical form); the Palisade membrane guards the **foreign outcome edge** (what we trust back from a boundary we do not own); and the **revetment** guards the **publication edge** (what a shipped face may say of a veiled interior). The revetment is the singular member — sovereign on *both* sides, the only *transmitting* membrane, shipping a faithful translation by design rather than bending to someone else's reality. It is not the veil family (VOr_q4f): the veil decides *which files leave*, the revetment *what a leaving file may say*. Vocabulary home is MCM Lapidary (`mcm_revetment`, `mcm_spall`, the four-invariant face law), not restated here.
