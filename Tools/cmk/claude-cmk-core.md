...
| **Skidmark** | `mcm_` | An antipattern: version commentary in a durable document — revision markers, dated change narration, "new in vX" notes, "(added <date>)" framings. Durable documents (spec, guide, code, context file — and paddock/docket own-voice prose) speak only in the present tense; history-time knowledge lives in the commit, where git log/blame carry it with a forcing function. Dated artifacts (memo, scar, commit message — and dated frozen records *quoted inside* a durable document) are exempt: history is their content. Detection and drain moves: ACG `ACGm_107`. |
...
Annotations for the acronym homes indexed in `claude-cmk-acronyms.md` — the per-row descriptions and family topology the index does not carry.
...
| **ACG** | `jjqs_studbook/guides/cmk/ACG-AllodialCodingGuide.md` | Allodial Coding Guide — source-side complement to MCM: reference the home, don't recreate (values→constants, concepts→quoin-refs). Veiled/proprietary move-catalog; guide-family sibling to BCG/RCG/WSG/CBG. |
| **GMG** | `jjqs_studbook/guides/cmk/GMG-GuideMetaGuide.md` | Guide Meta-Guide — the guide for writing guides: the canonical section skeleton and the family's shared framing conventions (foreign-environment sibling, two-genres split, cited-rule scheme). Homes the guide-family form once so a guide cites the convention rather than re-deriving it. The spec-side analogue of MCM/AXLA, for guides. |
| **AWG** | `jjqs_studbook/guides/cmk/AWG-AspirantWritingGuide.md` | Aspirant Writing Guide — how an agent–operator pair refines an aspirant sheaf toward settling under MCM/AXLA law and the JJS0 mulling-heat doctrine: charter-first, worked census, mint-follows-API, timeless prose, banked nudges, the settling register with drain modes. Guide-family sibling per GMG; expects delivery via the aspirant-sheaves tackle's trappings. |
