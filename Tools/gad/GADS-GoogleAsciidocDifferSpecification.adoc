...
. **Visual Assignment**: Each {gadis_change} receives a unique background color for visual correlation
...
User clicks operation button in Change #N
  ...
  → Button extracts route array from associated {gaddo_operation}
  → System scrolls target pane to route position
  → Colored background at destination matches Change #N color
----

==== Implementation Details

* **Color Consistency**: Each {gadis_change} maintains the same background color across all three panes
* **Independent Scrolling**: Panes scroll independently to support non-linear document exploration
* **Route Preservation**: Operation routes remain valid as they reference immutable DOM structures
...
The {gaddl_library} generates addElement, removeElement, relocateNode, and modifyTextElement operations with complete route array information for precise element positioning.

[[gaddo_dom]]
{gaddo_dom}::
The type representing any DOM structure that can be processed by {gaddl_library}.
A {gaddo_dom} is an in-memory tree representation of HTML with nodes accessible via route arrays.
...
Each entry contains the route array and character offsets needed to locate the word within its source DOM text node.
...
[[gaddo_operation]]
{gaddo_operation}::
The type representing an individual diff-dom operation with action type, route for DOM positioning, and change information (added/removed/modified content).
...
{gadic_annotation_entry}::
Individual annotation describing a specific {gaddo_operation}, including operation index, action type, route, and formatted payload representation.
...
[[gadvd_change_pane]]
{gadvd_change_pane}::
Bottom pane containing scrollable list of {gadvd_change_entry} elements with navigation controls for each {gadis_change}.

[[gadvd_change_entry]]
{gadvd_change_entry}::
Visual element in {gadvd_change_pane} representing one {gadis_change} with navigation buttons.
Contains {gadvd_operation_button} controls for each operation in the change (typically one per pane).

[[gadvd_operation_button]]
{gadvd_operation_button}::
Clickable control within {gadvd_change_entry} that scrolls target pane to operation's route location.
Forward operations target {gadvd_left_pane}, reverse operations target {gadvd_right_pane}.

[[gadvd_left_rendered]]
{gadvd_left_rendered}::
Rendered DOM for {gadvd_left_pane} with deletion styling (strikethrough) and change-specific background colors.

[[gadvd_right_rendered]]
{gadvd_right_rendered}::
Rendered DOM for {gadvd_right_pane} with addition styling (bold) and change-specific background colors.
...
Represents the operations needed to transform the "before" state into the "after" state, with all routes referencing positions in {gadic_from_dom}.
...
Represents the operations needed to transform the "after" state back into the "before" state, with all routes referencing positions in {gadic_to_dom}.
...
Correlates operations based on DOM proximity and inverse relationships (e.g., forward deletion paired with reverse addition), producing {gadis_change_list} as output.

[[gadis_change_rendering]]
{gadis_change_rendering}::
The process within {gadis_diff_sequence} that applies visual styling to each {gadis_change} for dual view display.
Assigns consistent background colors to each {gadis_change} and determines operation-specific visual treatments (strikethrough for deletions, bold for additions) to produce {gadvd_left_rendered} and {gadvd_right_rendered}.
