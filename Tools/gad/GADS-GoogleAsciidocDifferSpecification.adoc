...
// ⟦axc_step⟧
{gadkp_bounded_documents}::
The GAD system processes technical documentation files that are bounded in size, typically under 1MB when rendered to HTML. Modern systems with gigabytes of RAM can efficiently handle multiple DOM representations simultaneously, enabling dual view rendering without memory pressure.
...
The {gadt_manifest} JSON structure contains:

* [[gadmf_branch]]
// ⟦axj_field⟧
{gadmf_branch} - The top-level JSON field containing the Git branch name being processed, corresponds to the {gadfa_branch} argument passed to {gadt_factory}.

* [[gadmf_asciidoc]]
// ⟦axj_field⟧
{gadmf_asciidoc} - The top-level JSON field containing the source AsciiDoc filename, corresponds to the {gadfa_adoc_filename} argument passed to {gadt_factory}.

* [[gadmf_last_processed]]
// ⟦axj_field⟧
{gadmf_last_processed} - The top-level JSON field containing the most recently processed commit SHA, enables {gadt_factory} to detect new commits during incremental watch mode.

* [[gadmf_commits]]
// ⟦axj_field⟧
{gadmf_commits} - The top-level JSON array containing individual commit metadata objects, each with hash, timestamp, date, message, html_file, and html_sha256 fields.

Each commit entry in {gadmf_commits} contains:

** [[gadmf_hash]]
// ⟦axj_field⟧
{gadmf_hash} - The JSON field "hash" containing the full Git commit SHA, corresponds to the {gadda_commit_hash} argument passed to {gadp_distill}.

** [[gadmf_timestamp]]
// ⟦axj_field⟧
{gadmf_timestamp} - The JSON field "timestamp" containing the formatted processing timestamp in YYYYMMDDHHMMSS format for timestamp display and filename generation.

** [[gadmf_date]]
// ⟦axj_field⟧
{gadmf_date} - The JSON field "date" containing the commit date in YYYYMMDDHHMMSS format, corresponds to the {gadda_commit_date} argument passed to {gadp_distill}.

** [[gadmf_message]]
// ⟦axj_field⟧
{gadmf_message} - The JSON field "message" containing the Git commit message, corresponds to the {gadda_commit_message} argument passed to {gadp_distill}.

** [[gadmf_html_file]]
// ⟦axj_field⟧
{gadmf_html_file} - The JSON field "html_file" containing the generated HTML filename following the {gadif_html_commit} pattern for consistent file organization.

** [[gadmf_html_sha256]]
// ⟦axj_field⟧
{gadmf_html_sha256} - The JSON field "html_sha256" containing the SHA-256 hash of the entire normalized HTML file, enables {gadt_inspector} to identify unchanged commits for {gadir_unchanged_indicator} visual treatment.
...
The {gadt_inspector} executes diff analysis by creating immutable DOM structures from input HTML, running {gaddl_library} to compute operation lists, then applying CSS styling and generating visualization. On error, display error text in {gadip_rendered} instead of diff.
...
**Processing**: {gaddl_library} invoked twice to generate {gadis_forward_list} (before→after) and {gadis_reverse_list} (after→before), each containing individual {gaddo_operation} entries with complete {gadis_route} information for precise element positioning. {gadis_change_correlation} analyzes {gadis_forward_list} and {gadis_reverse_list} to group {gaddo_operation} entries by proximity and inverse relationships, producing {gadis_change_list}. CSS classes applied to construct view-specific rendered output. {gadic_annotation_collection} produced containing {gadic_annotation_entry} elements describing each {gadis_change}.

**Output**: {gadic_multiview_result} with pre-computed views for {gadip_tab_bar} display options ({gadi_prototype_view} and {gadi_dual_view} content), displayed in {gadip_rendered} pane.
...
// ⟦axc_step⟧
Run {gaddl_library} on immutable DOM trees to generate diff operations
...
Commits with unchanged {gadmf_html_sha256} values display with {gadir_unchanged_indicator} visual treatment in rail rows.
...
The {gadi_dual_view} provides a comprehensive side-by-side comparison interface for analyzing document changes between {gadic_from_dom} and {gadic_to_dom}. This architecture enables visual correlation of changes without requiring synchronized scrolling, allowing the {gadu_operator} to explore document differences at their own pace.
...
The dual view generation follows this sequence within {gadis_diff_sequence}:

. **Diff Analysis**: {gaddl_library} generates {gadis_forward_list} and {gadis_reverse_list}
. **Change Correlation**: {gadis_change_correlation} groups related operations into {gadis_change} instances
. **Visual Assignment**: Each {gadvd_change_entry} receives a unique background color for visual correlation
. **Rendering**: {gadis_change_rendering} produces:
  - {gadvd_left_rendered}: {gadic_from_dom} with deletion styling (strikethrough + color)
  - {gadvd_right_rendered}: {gadic_to_dom} with addition styling (bold + color)
  - {gadvd_change_pane}: List of {gadvd_change_entry} elements with {gadvd_operation_button} controls

==== User Interaction Model

The dual view enables three primary interaction patterns:

. **Direct Pane Exploration**: Scroll {gadvd_left_pane} and {gadvd_right_pane} independently to examine document structure
. **Change-Driven Navigation**: Click {gadvd_operation_button} in {gadvd_change_pane} to jump to specific change locations
...
* **Color Consistency**: Each {gadvd_change_entry} maintains the same background color across all three panes
* **Independent Scrolling**: Panes scroll independently to support non-linear document exploration
* **Route Preservation**: Operation {gadis_route} values remain valid as they reference immutable DOM structures
* **Eager Computation**: Per {gadkp_bounded_documents}, all views are pre-computed and stored in {gadic_multiview_result}
* **Instant Switching**: {gadiu_tab_switch} swaps pre-computed HTML without re-running diff analysis
...
The {gaddl_library} generates addElement, removeElement, relocateNode, and modifyTextElement operations with complete {gadis_route} information for precise element positioning.
