...
// ⟦axt_type⟧
{gaddo_dom}::
The type representing any DOM structure that can be processed by {gaddl_library}.
A {gaddo_dom} is an in-memory tree representation of HTML with nodes accessible via {gadis_route} arrays.

[[gaddo_word_flat]]
// ⟦axt_type⟧
{gaddo_word_flat}::
The type representing a flattened, change-suppressed, word-per-line text extraction from a {gaddo_dom}.
...
// ⟦axt_type⟧
{gaddo_word_map}::
The type representing a mapping structure that correlates each line in a {gaddo_word_flat} back to its source position in the original {gaddo_dom}.
Each entry contains the {gadis_route} and character offsets needed to locate the word within its source DOM text node.

[[gadic_from_dom]]
// ⟦axt_type⟧
{gadic_from_dom}::
The specific {gaddo_dom} instance parsed from the "From" {gadt_page}, used as the baseline input to {gaddl_library} analysis.

[[gadic_to_dom]]
// ⟦axt_type⟧
{gadic_to_dom}::
The specific {gaddo_dom} instance parsed from the "To" {gadt_page}, used as the target input to {gaddl_library} analysis.

[[gaddo_operation_list]]
// ⟦axt_type⟧
{gaddo_operation_list}::
The type representing a list of diff operations that can be generated by {gaddl_library} analysis, containing ordered {gaddo_operation} entries.

[[gaddo_operation]]
// ⟦axt_type⟧
{gaddo_operation}::
The type representing an individual diff-dom operation with action type, {gadis_route} for DOM positioning, and change information (added/removed/modified content).

[[gadic_rendered_dom]]
// ⟦axt_type⟧
{gadic_rendered_dom}::
Output DOM structure with applied CSS classes for diff visualization. Used primarily in {gadi_prototype_view} to show the "to" document with visual diff annotations.

[[gadic_annotation_collection]]
// ⟦axt_type⟧
{gadic_annotation_collection}::
Complete set of operation annotations generated during rendering, provides human-readable descriptions for each {gaddo_operation} in the visualization.

[[gadic_annotation_entry]]
// ⟦axt_type⟧
{gadic_annotation_entry}::
Individual annotation describing a specific {gaddo_operation}, including operation index, action type, {gadis_route}, and formatted payload representation.
...
// ⟦axo_guide⟧
{gadvd_left_pane}::
Left pane displaying {gadic_from_dom} with deletion indicators and color-coded change backgrounds.

[[gadvd_right_pane]]
// ⟦axo_guide⟧
{gadvd_right_pane}::
Right pane displaying {gadic_to_dom} with addition indicators and color-coded change backgrounds.

[[gadvd_change_pane]]
// ⟦axo_guide⟧
{gadvd_change_pane}::
Bottom pane containing scrollable list of {gadvd_change_entry} elements with navigation controls.
Entries ordered by {gadis_reverse_list} operation sequence, reflecting {gadic_to_dom} layout since {gadvd_right_pane} always displays the target/improved specification state.

[[gadvd_change_entry]]
// ⟦axo_guide⟧
{gadvd_change_entry}::
Visual element in {gadvd_change_pane} representing one {gadis_change} with navigation buttons.
Contains {gadvd_operation_button} elements created for each {gaddo_operation} in the {gadis_change}: {gadis_reciprocal_change} entries have two buttons (one for each pane), {gadis_monocular_change} entries have one button for the appropriate pane.

[[gadvd_operation_button]]
// ⟦axo_guide⟧
{gadvd_operation_button}::
Clickable control within {gadvd_change_entry} that scrolls target pane to operation's {gadis_route} location.
Forward operations target {gadvd_left_pane}, reverse operations target {gadvd_right_pane}.

[[gadvd_left_rendered]]
// ⟦axt_type⟧
{gadvd_left_rendered}::
{gadic_from_dom} prepared for {gadvd_left_pane} display with deletion styling (strikethrough) and change-specific background colors applied.
Serves as the prepared structure for the left side of {gadi_dual_view}.

[[gadvd_right_rendered]]
// ⟦axt_type⟧
{gadvd_right_rendered}::
{gadic_to_dom} prepared for {gadvd_right_pane} display with addition styling (bold) and change-specific background colors applied.
Serves as the prepared structure for the right side of {gadi_dual_view}, always displaying the target/improved specification state.
...
// ⟦axo_guide⟧
{gadt_inspector}::
An HTML+JavaScript application served by {gadt_factory} via HTTP that loads two {gadt_page_s} and displays their differences using {gaddl_library}.
...
Commits where {gadmf_html_sha256} matches the previous commit display with {gadir_unchanged_indicator} visual treatment.
...
Tab content switches based on {gadis_tab_state} between {gadi_prototype_view}, {gadi_dual_view}, and other visualization modes via {gadiu_tab_switch} actions.
...
// ⟦axt_type⟧
{gadis_forward_list}::
The specific {gaddo_operation_list} produced by running {gaddl_library} with {gadic_from_dom} as source and {gadic_to_dom} as target.
Represents the operations needed to transform the "before" state into the "after" state, with all {gadis_route} values referencing positions in {gadic_from_dom}.

[[gadis_reverse_list]]
// ⟦axt_type⟧
{gadis_reverse_list}::
The specific {gaddo_operation_list} produced by running {gaddl_library} with {gadic_to_dom} as source and {gadic_from_dom} as target.
Represents the operations needed to transform the "after" state back into the "before" state, with all {gadis_route} values referencing positions in {gadic_to_dom}.

[[gadis_change_correlation]]
// ⟦axo_pattern⟧
{gadis_change_correlation}::
The process within {gadis_diff_sequence} that analyzes {gadis_forward_list} and {gadis_reverse_list} to group related operations into {gadis_change} instances.
...
Paired operations become {gadis_reciprocal_change} instances; unpaired operations become separate {gadis_monocular_change} instances.
Produces {gadis_change_list} as output.

[[gadis_change_rendering]]
// ⟦axo_pattern⟧
{gadis_change_rendering}::
The process within {gadis_diff_sequence} that applies visual styling to each {gadvd_change_entry} for dual view display.
Assigns background colors using a two-color scheme: one color for {gadis_reciprocal_change} entries, a distinct color for {gadis_monocular_change} entries.
Determines operation-specific visual treatments (strikethrough for deletions, bold for additions) to produce {gadvd_left_rendered} and {gadvd_right_rendered}.

[[gadis_change]]
// ⟦axt_type⟧
{gadis_change}::
A correlated group of {gaddo_operation} entries from {gadis_forward_list} and {gadis_reverse_list} that together represent a single logical modification between {gadic_from_dom} and {gadic_to_dom}.
Operations within a {gadis_change} are affiliated by DOM proximity and inverse relationships (e.g., forward deletion paired with reverse addition).

[[gadis_change_list]]
// ⟦axt_type⟧
{gadis_change_list}::
The specific list of correlated changes produced by analyzing {gadis_forward_list} and {gadis_reverse_list} for the current {gadic_from_dom} and {gadic_to_dom} pair.
Each {gadis_change} entry represents a logical change identified between these specific document versions.

[[gadis_reciprocal_change]]
// ⟦axt_type⟧
{gadis_reciprocal_change}::
A {gadis_change} consisting of exactly one {gaddo_operation} from {gadis_forward_list} paired with exactly one {gaddo_operation} from {gadis_reverse_list}.
The paired operations are reciprocals of each other, representing a single unambiguous logical modification between {gadic_from_dom} and {gadic_to_dom}.
{gadis_reciprocal_change} instances form the primary case in {gadis_change_correlation}.

[[gadis_monocular_change]]
// ⟦axt_type⟧
{gadis_monocular_change}::
A {gadis_change} consisting of operations from only one side: either {gadis_forward_list} or {gadis_reverse_list}, but not both.
Created when {gaddo_operation} entries cannot be clearly paired during {gadis_change_correlation} analysis.
Each unpaired operation from either side generates a separate {gadis_monocular_change} instance containing that single operation.
...
A {gadis_route} enables precise element location within immutable DOM structures, serving as the addressing mechanism for {gaddo_operation} positioning and navigation from {gadvd_change_entry} controls to document locations.
...
The {gadir_unchanged_indicator} helps the {gadu_operator} identify which commits produced no rendering changes while maintaining full git history visibility.
