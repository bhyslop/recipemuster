...
[[gadi_dual_view]]
{gadi_dual_view}::
Side-by-side comparison view showing "before" and "after" content independently for visual comparison.
Displays changes using {gadvd_left_pane}, {gadvd_right_pane}, and {gadvd_change_pane} with consistent color coding for each {gadis_change}, allowing visual correlation without scroll synchronization.

[[gadvd_left_pane]]
{gadvd_left_pane}::
The left pane in {gadi_dual_view} displaying {gadic_from_dom} with visual indicators for content that was removed or modified.
Shows the "before" state with deletions marked using background colors corresponding to each {gadis_change}. Scrolls independently from {gadvd_right_pane}.

[[gadvd_right_pane]]
{gadvd_right_pane}::
The right pane in {gadi_dual_view} displaying {gadic_to_dom} with visual indicators for content that was added or modified.
Shows the "after" state with additions marked using background colors corresponding to each {gadis_change}. Scrolls independently from {gadvd_left_pane}.

[[gadvd_change_pane]]
{gadvd_change_pane}::
The bottom pane in {gadi_dual_view} displaying a scrollable list of {gadvd_change_entry} elements, one for each {gadis_change} in the current comparison.
Provides a navigable overview of all changes, allowing the {gadu_operator} to click {gadvd_operation_button} controls to scroll to specific change locations in {gadvd_left_pane} or {gadvd_right_pane}.

[[gadvd_change_entry]]
{gadvd_change_entry}::
Visual representation of a single {gadis_change} within {gadvd_change_pane}.
Each {gadvd_change_entry} corresponds to exactly one {gadis_change} and contains one or more {gadvd_operation_button} controls representing the {gaddo_operation} elements from {gadis_forward_list} and {gadis_reverse_list} that comprise the change.
Typically contains two operation buttons (one targeting {gadvd_left_pane}, one targeting {gadvd_right_pane}), though complex changes may include additional operations.

[[gadvd_operation_button]]
{gadvd_operation_button}::
Clickable control within a {gadvd_change_entry} representing a single {gaddo_operation}.
When clicked, scrolls the appropriate pane to the location specified by the operation's route: operations from {gadis_forward_list} scroll {gadvd_left_pane} to positions in {gadic_from_dom}, while operations from {gadis_reverse_list} scroll {gadvd_right_pane} to positions in {gadic_to_dom}.

[[gadvd_left_rendered]]
{gadvd_left_rendered}::
The finalized rendered DOM for {gadvd_left_pane} produced by {gadis_change_rendering}.
Contains {gadic_from_dom} with visual styling applied including background colors for each {gadis_change} and strikethrough for deleted content.

[[gadvd_right_rendered]]
{gadvd_right_rendered}::
The finalized rendered DOM for {gadvd_right_pane} produced by {gadis_change_rendering}.
Contains {gadic_to_dom} with visual styling applied including background colors for each {gadis_change} and bold/highlighting for added content.
