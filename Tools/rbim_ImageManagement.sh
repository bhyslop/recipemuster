...
zrbim_kindle() {
  ...
  # Temp files for build operations
...
zrbim_verify_git_clean() {
  ...
  git config --get remote.origin.url > "${ZRBIM_GIT_REPO_FILE}" || bcu_die "Failed to get repo URL"
...
zrbim_submit_build() {
  ...
  sed -i 's|.*/||' "${ZRBIM_BUILD_ID_FILE}" || bcu_die "Failed to parse build ID"
...
zrbim_wait_build_completion() {
  ...
  while true; do
    ...
    if test "${z_status}" != "PENDING"; then
      if test "${z_status}" != "QUEUED"; then
        if test "${z_status}" != "WORKING"; then
          break
        fi
      fi
    fi
...
zrbim_validate_moniker_predicate() {
  ...
  # Check format: lowercase alphanumeric with dash/underscore
