...
| Tinder constant (public)     | `«PREFIX»_«name»`            | `RBCC_rbrr_file`             | Impl     | lower_snake (multi-word)      |
...
The band lives as tinder in `bubc_constants.sh` — `BUBC_band_base` and `BUBC_band_width` fix the range, placed clear of shell-reserved codes (2, 126–128+n), the `sysexits.h` range (64–78), and `timeout`/container-runtime codes (124–125). Both primitives read that tinder, so a code is in-band only because it was minted there. The ceiling at 124 fixes the band's maximum extent at 100–123, and `BUBC_band_width=24` claims that whole window — the terminal width, never to widen again. When the band fills, capacity comes from the allocation rule (share across alternatives), never from growth.

One overlap is deliberate: **curl**. When the band was placed, curl's exit codes topped out at 92; curl 8.6.0 (2024-01) minted `CURLE_TOO_LARGE` = 100 and 8.8.0 (2024-05) `CURLE_ECH_REQUIRED` = 101 — squarely on `BUBC_band_regime`/`BUBC_band_enroll` — and curl keeps minting at roughly one code a year. Re-basing the band was weighed and declined: the window under the 124 ceiling is zero-sum (headroom below curl trades one-for-one against free codes above), a move renumbers the whole census, and it rents years only. Containment is the resolution, and it is normative: **a curl exit status is captured and classified at the call site** (`|| z_curl_status=$?`, then branch on the value) so it never reaches the band membrane; **a bare `curl … || buc_die` chain is rule-barred** — it would hand a curl exit straight to the membrane, where 100/101 masquerade as in-band rejections.
