...
Suites enable sweep dispatch: run a cross-cutting subset of cases grouped by their owning fixtures.
Suite context is set with `butr_suite_enroll(SUITE1 [SUITE2...])` before case enrollments.
Queried by `butd_run_sweep(suite_name)`.
...
* `butd_run_sweep(suite)` — cross-cutting suite, grouped by fixture
