...
zbutcfc_seed_under() {
  ...
  local -r z_name="${2}"
  local -r z_value="${3}"
  printf '%s\n' "${z_value}" > "${z_dir}/${z_name}" || buto_fatal_now "seed failed: ${z_dir}/${z_name}"
}
...
zbutcfc_seed_previous() {
  local -r z_name="${1}"
  local -r z_value="${2}"
  mkdir -p "${BURD_PREVIOUS_DIR}" || buto_fatal_now "mkdir previous failed"
  zbutcfc_seed_under "${BURD_PREVIOUS_DIR}" "${z_name}" "${z_value}"
}
...
butcfc_relay_forwards_tcase() {
  ...
  buf_relay || buto_fatal_now "buf_relay failed"

  test -f "${BURD_OUTPUT_DIR}/butcfc_fwd" || buto_fatal_now "buf_relay did not forward butcfc_fwd"
  local -r z_got=$(<"${BURD_OUTPUT_DIR}/butcfc_fwd")
  test "${z_got}" = "forwarded-value" || buto_fatal_now "forwarded content mismatch: '${z_got}'"
}
...
butcfc_relay_preserves_current_tcase() {
  ...
  zbutcfc_seed_under "${BURD_OUTPUT_DIR}"   "butcfc_pres" "from-current"

  buf_relay || buto_fatal_now "buf_relay failed"

  local -r z_got=$(<"${BURD_OUTPUT_DIR}/butcfc_pres")
  test "${z_got}" = "from-current" \
    || buto_fatal_now "buf_relay clobbered an existing current file: '${z_got}'"
}
...
butcfc_relay_idempotent_tcase() {
  ...
  buf_relay || buto_fatal_now "buf_relay first call failed"
  buf_relay || buto_fatal_now "buf_relay second call failed"

  local -r z_got=$(<"${BURD_OUTPUT_DIR}/butcfc_idem")
  test "${z_got}" = "v1" || buto_fatal_now "idempotent relay altered content: '${z_got}'"
}
...
butcfc_read_fact_tcase() {
  ...
  local z_value
  z_value=$(buf_read_fact_capture "butcfc_greeting") || buto_fatal_now "buf_read_fact_capture failed on present fact"
  test "${z_value}" = "hello world" \
    || buto_fatal_now "buf_read_fact_capture returned '${z_value}' expected 'hello world'"
}
...
butcfc_read_fact_absent_tcase() {
  ...
  test "${z_status}" -ne 0 \
    || buto_fatal_now "buf_read_fact_capture should fail on an absent fact"
}
...
butcfc_elect_express_tcase() {
  ...
  z_value=$(buf_elect_fact_capture "express-wins" "butcfc_definitely_absent_fact") \
    || buto_fatal_now "buf_elect_fact_capture failed with a non-empty express value"
  test "${z_value}" = "express-wins" \
    || buto_fatal_now "buf_elect_fact_capture returned '${z_value}' expected 'express-wins'"
}
...
butcfc_elect_after_relay_tcase() {
  ...
  z_value=$(buf_elect_fact_capture "" "butcfc_baton") \
    || buto_fatal_now "buf_elect_fact_capture failed after a relay"
  test "${z_value}" = "baton-value" \
    || buto_fatal_now "elect after relay returned '${z_value}' expected 'baton-value'"

  # The relay forwarded the baton into current/, ready for the next promotion.
  test -f "${BURD_OUTPUT_DIR}/butcfc_baton" \
    || buto_fatal_now "relay left no forwarded copy in current/"
  local -r z_fwd=$(<"${BURD_OUTPUT_DIR}/butcfc_baton")
  test "${z_fwd}" = "baton-value" || buto_fatal_now "forwarded baton mismatch: '${z_fwd}'"
}
...
butcfc_chain_survives_consumption_tcase() {
  ...
  local z_value
  z_value=$(buf_read_fact_capture "butcfc_survive") || buto_fatal_now "first read failed"
  test "${z_value}" = "immortal" || buto_fatal_now "first read returned '${z_value}'"

  # A second read succeeds identically — the first read consumed nothing.
  z_value=$(buf_read_fact_capture "butcfc_survive") || buto_fatal_now "second read failed"
  test "${z_value}" = "immortal" || buto_fatal_now "second read returned '${z_value}'"
  ...
  test -f "${BURD_PREVIOUS_DIR}/butcfc_survive" \
    || buto_fatal_now "previous/ fact vanished under consumption"
  local -r z_fwd=$(<"${BURD_OUTPUT_DIR}/butcfc_survive")
  test "${z_fwd}" = "immortal" || buto_fatal_now "relayed current/ copy mismatch: '${z_fwd}'"
}
...
butcfc_elect_chain_tcase() {
  ...
  z_value=$(buf_elect_fact_capture "" "butcfc_chained") \
    || buto_fatal_now "buf_elect_fact_capture failed falling back to a present chained fact"
  test "${z_value}" = "from-chain" \
    || buto_fatal_now "buf_elect_fact_capture returned '${z_value}' expected 'from-chain'"
  ...
  test "${z_status}" -ne 0 \
    || buto_fatal_now "buf_elect_fact_capture should fail when express is empty and the fact is absent"
}
