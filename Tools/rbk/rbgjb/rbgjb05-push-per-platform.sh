...
#                _RBGY_GAR_HOST_SUFFIX, _RBGY_ARK_SUFFIX_ATTEST
#
# Pushes each -attest-{arch} tag to registry so they are available for the
# CB images: field SLSA provenance generation.
...
for SUFFIX in "${SUFFIXES[@]}"; do
  ...
  echo "Pushing: ${IMAGE_BASE}:${ATTEST_TAG}"
  docker push "${IMAGE_BASE}:${ATTEST_TAG}"
done
