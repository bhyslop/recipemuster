...
RBRN assignment files are found in `.rbk/rbrn_*.env` and configure container deployment and network security.
...
Semicolon-delimited Docker volume mount specifications.
Each entry is passed verbatim as a `--volume` argument.
Empty string means no volume mounts.
Active when {rbrn_runtime} is {rbrn_runtime_docker}.
0-240 characters.

//axhrgv_variable
{rbrn_podman_volume_mounts}

Semicolon-delimited Podman volume mount specifications.
Each entry is passed verbatim as a `--volume` argument.
Podman-specific options (`:U`, `:O`, `idmap`) are valid here.
Empty string means no volume mounts.
Active when {rbrn_runtime} is {rbrn_runtime_podman}.
0-240 characters.
