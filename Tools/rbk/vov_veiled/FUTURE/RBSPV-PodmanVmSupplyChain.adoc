...
These variables are defined in the RBRM (Machine) regime at `.rbk/rbrm.env`, separated from the broader RBRR configuration to clarify the distinction between Recipe Bottle's application-level configuration and the underlying VM infrastructure configuration.
