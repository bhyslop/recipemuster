...
{rbtr_payor}
OAuth access by refreshing the stored OAuth token and then calling the Cloud Resource Manager projects.get API on the {rbtgi_manor}.
...
A successful response confirms both that the refresh token remains valid and that the {rbtgi_manor} is accessible via CRM.
...
{rbtgo_payor_refresh})"
.... If `«HTTP_STATUS»` is one of 500, 502, 503, or 504 (transient server-side error):
..... Re-execute the {rbbc_call} up to 3 total attempts, sleeping with exponential backoff between attempts (2s, then 4s)
..... On any non-5xx response within the retry window, re-enter *Evaluate Response* with that status
..... {rbbc_fatal}
if all 3 attempts return a 5xx status: "Iteration «I»: Payor repeated transient HTTP «HTTP_STATUS» after 3 attempts"
.... Otherwise:
..... {rbbc_fatal}
