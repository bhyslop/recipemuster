//axvd_sheaf axd_normative
This is a normative portion of the {rbs0_imprimatur}: binding — code, tests, and decisions may rely on it.
//axhob_operation
{rbtgo_depot_unmake}

The
{rbtr_payor}
destroys an
{rbtgi_depot}
and all contained resources:

//axhopt_typed_parameter
{rbtgi_depot_project}
. *Empty-argument refusal*:
.. {rbbc_fatal}
if no argument supplied: print operator pointer to
{rbtgo_depot_list}
for candidate enumeration and exit non-zero.
No auto-routing to list — single responsibility preserved.
. *Safety Confirmation*:
.. If `RBGP_CONFIRM_DESTROY` environment variable is set:
... {rbbc_require}
value matches the depot project ID argument
... Proceed without interactive confirmation
.. Otherwise:
... Display comprehensive destruction warnings
... {rbbc_require}
operator confirmation with exact depot project ID (interactive)

//axhos_step
. *Refuse live depot target*:
.. {rbbc_fatal}
if argument equals the live depot derivation
`{rbrd_cloud_prefix}{rbgc_depot_project_infix}{rbrd_depot_moniker}`
(i.e., the value materialised as `RBDC_DEPOT_PROJECT_ID`).
Recovery: rename
{rbrd_depot_moniker}
in the {rbrr_regime}, or run marshal-zero (`rbw-MZ`), then retry.
.. Local string comparison; zero network traffic. Defense against orphaning local regime state by destroying the depot the regime currently points to.

//axhos_step
. *Authenticate as
{rbtr_payor}*:
.. Execute
{rbtoe_payor_authenticate}
.. {rbbc_require}
authentication successful

//axhos_step
. *Validate Target Depot*:
.. {rbbc_require}
depot project ID provided as first argument
.. {rbbc_store}
`«DEPOT_PROJECT_ID»` from argument
.. {rbbc_call}
link:https://cloud.google.com/resource-manager/reference/rest/v3/projects/get[projects.get] for `«DEPOT_PROJECT_ID»`
.. {rbbc_require}
project exists and state is `ACTIVE`
.. {rbbc_fatal}
if state is already `DELETE_REQUESTED`: "Project already marked for deletion"
.. *DisplayName discriminator* — symmetric with
{rbtgo_depot_list}:
... {rbbc_store}
`«DISPLAY_NAME»` from response `.displayName`
... {rbbc_require}
`«DISPLAY_NAME»` starts with the anchor literal `«RBGC_DEPOT_DISPLAY_PREFIX» `; otherwise
{rbbc_fatal}
with diagnostic naming the mismatch and pointing operator at
{rbtgo_depot_list}
for candidate enumeration. Refuses non-depot projects in the
{rbtr_payor}
account, the structural defense once unmake is decoupled from
{rbrr_regime}.

//axhos_step
. *Check for and remove
{gcp_lien_s}*:
.. {rbbc_call}
link:https://cloud.google.com/resource-manager/reference/rest/v1/liens/list[liens.list] to list existing liens on resource: `projects/«DEPOT_PROJECT_ID»`
.. {rbbc_store}
`«LIEN_LIST»` from response `.liens[]`
.. For each lien in `«LIEN_LIST»`:
... {rbbc_call}
link:https://cloud.google.com/resource-manager/reference/rest/v1/liens/delete[liens.delete] to delete lien using `.name` field
... {rbbc_require}
HTTP 200 response for each lien deletion
.. {rbbc_require}
all liens removed before proceeding

//axhos_step
. *Unlink billing account*:
.. {rbbc_call}
link:https://cloud.google.com/billing/docs/reference/rest/v1/projects/updateBillingInfo[cloudbilling.projects.updateBillingInfo] with body `{"billingAccountName":""}` for `projects/«DEPOT_PROJECT_ID»`
.. {rbbc_require}
billing unlink successful (HTTP 200)
.. Note: Unlinking BEFORE deletion releases billing quota immediately; deletion without unlink leaves quota consumed for 30-day retention period

//axhos_step
. *Delete Dual Worker Pools*:
.. For each pool suffix (`-tether`, `-airgap`):
... Construct pool resource name: `projects/«DEPOT_PROJECT_ID»/locations/{rbrd_gcp_region}/workerPools/rbw-«INPUT_DEPOT_NAME»-pool«SUFFIX»`
... {rbbc_call}
link:https://cloud.google.com/build/docs/api/reference/rest/v1/projects.locations.workerPools/delete[workerPools.delete] for the pool resource name
... Treat HTTP 404 as success (pool already gone — tolerated)
.. {rbbc_require}
HTTP 200 or 404 response for both pools; any other status is {rbbc_fatal}

//axhos_step
. *Initiate depot deletion*:
.. {rbbc_submit}
link:https://cloud.google.com/resource-manager/reference/rest/v3/projects/delete[projects.delete] to execute project deletion for `projects/«DEPOT_PROJECT_ID»`
.. {rbbc_store}
`«DELETE_OPERATION»` from response if Long Running Operation returned
.. {rbbc_await}
`«DELETE_OPERATION»` if applicable
.. {rbbc_require}
deletion initiated successfully

//axhos_step
. *Verify deletion transition*:
.. {rbbc_call}
link:https://cloud.google.com/resource-manager/reference/rest/v3/projects/get[projects.get] to query project state for `projects/«DEPOT_PROJECT_ID»`
.. {rbbc_poll}
project state with 5-second intervals for up to 60 seconds
.. {rbbc_require}
project state equals `DELETE_REQUESTED`

//axhoc_completion
Successful execution destroys depot infrastructure.

NOTE: Depot deletion automatically removes all infrastructure created by
{rbtgo_depot_levy}:
the
{rbtr_mason}
service account,
{rbtgi_repository},
and any
{rbtr_governor},
{rbtr_director},
or
{rbtr_retriever}
service accounts created within the depot.
Both worker pools (tether and airgap) are explicitly deleted before project deletion because GCP project deletion does not automatically cascade to worker pool resources — an undrained pool can block or delay the delete operation.

NOTE: Project enters 30-day retention period in
{gcp_delete_requested}
state.
Use
{rbbc_call}
link:https://cloud.google.com/resource-manager/reference/rest/v3/projects/undelete[projects.undelete] to recover project if needed within retention window.
