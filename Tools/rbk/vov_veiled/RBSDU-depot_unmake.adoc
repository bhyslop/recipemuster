...
`{rbrr_cloud_prefix}{rbgc_depot_project_infix}{rbrr_depot_moniker}`
(i.e., the value materialised as `RBDC_DEPOT_PROJECT_ID`).
Recovery: rename
{rbrr_depot_moniker}
in `rbrr.env`, or run marshal-zero (`rbw-MZ`), then retry.
...
... Construct pool resource name: `projects/«DEPOT_PROJECT_ID»/locations/{rbrr_gcp_region}/workerPools/rbw-«INPUT_DEPOT_NAME»-pool«SUFFIX»`
... {rbbc_call}
