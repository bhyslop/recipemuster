...
.. NOTE: Format and joint length of `${RBRD_CLOUD_PREFIX}` + `d-` + `${RBRD_DEPOT_MONIKER}` are enforced upstream by `zrbrd_enforce` at regime kindle time, so this step performs no shape validation.
...
.. Use display name: `«RBGC_DEPOT_DISPLAY_PREFIX» governor «RBRD_DEPOT_MONIKER»`
.. {rbbc_store}
...
{rbtr_payor})
// roles/cloudbuild.connectionViewer eliminated — CB v2 connections no longer used
.. Set policy version: 3 for conditional binding support
.. {rbbc_require}
IAM policy update successful
.. NOTE: The `roles/owner` grant propagates to project-scope and SA-scope IAM caches effectively immediately, but resource-scope IAM caches (AR repo, bucket, secret) may lag.  Downstream operations that target resource-scope endpoints (notably {rbtgo_director_enrobe}'s AR repo IAM step) absorb this lag via the class C propagation tolerance in the grant harness.  See <<rbscig_iam_grant_contracts>> propagation catalog.
...
// CB v2 Connection Viewer verification step eliminated — CB v2 connections no longer used
