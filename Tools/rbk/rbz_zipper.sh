...
# rbz_emit_consts() - Emit the complete generated RBTDGC_ consts file to stdout.
...
# Three sections under one banner: the colophon group (this module binds the RBK
# projection prefixes — const name is RBTDGC_ + the enroll varname with its
# RBZ_ prefix stripped), the rbcc single-homed set, and the rbpc freehold
# test-rig set. rbcc_emit_consts is reachable here because every caller (rbq, the
# build, rblm) sources rbcc alongside rbz; rbpc_emit_consts by the same
# arrangement — rbq and the build source rbpc_constants.sh alongside rbcc and rbz.
rbz_emit_consts() {
  zrbz_sentinel
  printf '%s\n' "// Generated by the theurge build — rbz colophons + rbcc + rbpc constants. Do not edit."
  printf '%s\n' "// Regenerate: tt/rbw-tb.Build.sh"
  printf '%s\n' ""
  buz_emit_colophon_consts "RBTDGC_" "RBZ_" || buc_die "rbz_emit_consts: colophon emit failed"
  printf '%s\n' ""
