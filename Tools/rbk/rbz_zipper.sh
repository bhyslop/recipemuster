...
# Four sections under one banner: the colophon block (buz_emit_colophon_consts
...
# the rbcc single-homed set, the rbpc freehold test-rig set, and the rbgc
# propagation budget. rbcc_emit_consts is reachable here because every caller
# (rbq, the build) sources rbcc alongside rbz; rbpc_emit_consts and
# rbgc_emit_consts by the same arrangement. The BUWGC_ block is present only
