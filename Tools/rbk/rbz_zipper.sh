...
zrbz_kindle() {
  ...
  buz_enroll RBZ_THEURGE_SUITE    "rbw-ts"  "${z_mod}" "rbte_suite"   "imprint"  "Run a named test suite (reveille/picket/bivouac/echelon/gauntlet/skirmish/dogfight/siege/blockade)"
...
# (rbz_generate_consts) and rbq's freshness gate so the two never diverge.
# Three sections under one banner: the colophon block (buz_emit_colophon_consts
# walks every declared tome — RB's RBTDGC_ run, and the BUK zipper's BUWGC_ run
# when it is kindled alongside, each tome carrying its own add/strip prefixes),
# the rbcc single-homed set, and the rbpc freehold test-rig set. rbcc_emit_consts
# is reachable here because every caller (rbq, the build) sources rbcc alongside
# rbz; rbpc_emit_consts by the same arrangement. The BUWGC_ block is present only
# when the caller has kindled the BUK zipper (the build and rbq both do).
rbz_emit_consts() {
  zrbz_sentinel
  printf '%s\n' "// Generated by the theurge build — rbz + buwz colophons + rbcc + rbpc constants. Do not edit."
