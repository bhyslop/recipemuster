...
if [ "${_RBGV_VESSEL_MODE}" = "rbnve_conjure" ]; then
  ...
  printf '%s\n' '-----BEGIN PUBLIC KEY-----' > /workspace/keys/google-hosted-worker.pub
  printf '%s\n' 'MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEg9KII7kzr/30HBluf00y9WwtMFkE' >> /workspace/keys/google-hosted-worker.pub
  printf '%s\n' 'qc3oCcFVH3QJ37IBLUv/MUApbnNHFfD75ayJ/a0F45xa+MLv5zoep+GxsA==' >> /workspace/keys/google-hosted-worker.pub
  printf '%s\n' '-----END PUBLIC KEY-----' >> /workspace/keys/google-hosted-worker.pub
