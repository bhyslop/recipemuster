...
rba_novate_sederunt() {
  zrba_sentinel
  buc_doc_brief "Novate the sederunt — open a fresh full-window sederunt, extinguishing any standing one (the runway gate's named remedy)"
  buc_doc_shown || return 0

  buc_step "Novation — force-fresh sederunt against the RBRF trust"
  ...
  local z_token
  z_token=$(zrba_sederunt_read_capture) || buc_die_now "Sederunt not readable after novation"
  test -n "${z_token}" || buc_die_now "Sederunt holds an empty federated token"
  buc_success "Sederunt novated — fresh full-window federated token obtained (${#z_token} chars)"
}
...
rba_espy_sederunt() {
  zrba_sentinel
  buc_doc_brief "Espy the sederunt — report liveness and remaining runway from the cache alone (read-only: never opens, never prompts, no network)"
  buc_doc_shown || return 0

  buc_step "Espy — sederunt state against the RBRF trust"
  rbcc_source_active_rbrf
  source "${RBCC_rbrw_file}" || buc_die_now "Failed to source RBRW: ${RBCC_rbrw_file}"
  zrbrf_kindle
  zrbrw_kindle
  ...
  local z_path
  z_path=$(zrba_sederunt_path_capture) || buc_die_now "Failed to resolve the sederunt cache path"
  ...
  if test ! -f "${z_path}"; then
    ...
    z_runway=$(zrba_sederunt_runway_capture) || buc_die_now "Sederunt cache present but unreadable: ${z_path}"
...
runway=${z_runway}"
  buf_write_fact_multi "${RBRR_ACTIVE_FOEDUS}" "${RBCC_fact_ext_sederunt}" "${z_value}" \
    || buc_die_now "Failed to write the sederunt fact"

  if test "${z_verdict}" = "live"; then
    buc_success "Sederunt LIVE — runway ${z_runway}s (~$(( z_runway / 3600 ))h$(( (z_runway % 3600) / 60 ))m remaining)"
  else
    buc_warn "No live sederunt — verdict '${z_verdict}'; open one with any federated command or rbw-aN (fresh full window)"
  fi
}
