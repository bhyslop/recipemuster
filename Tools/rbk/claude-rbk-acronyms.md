...
- **RBGP** → `rbk/rbgp_payor.sh` (Payor — also homes the polity admission verbs `rbgp_brevet` / `rbgp_unseat` / `rbgp_attaint` / `rbgp_rehearse` over token-agnostic `zrbgp_*_core` helpers, dispatched via `rbgp_cli` and wielded as a donned governor mantle (`rba_compear` then `rba_don_capture governor`); colophons `rbw-pB`/`rbw-pU`/`rbw-pA`/`rbw-pr`. The interim payor-credentialed admission-composition proof is `rbgp_admission_proof` (`rbw-pP`), driven by the `admission-proof` service fixture; its standing-freehold sibling is `rbgp_freehold_proof` (`rbw-pF`) — ensure-brevet the single freehold subject (`RBPC_freehold_subject`) onto every mantle, then assert the manor-wide roster carries it — driven by the `foedus-freehold` service fixture (the quota-flat static counterpart to `foedus-lifecycle`). Contracts: RBSPB/RBSPU/RBSPA/RBSPO.)
