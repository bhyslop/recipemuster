...
- **RBSFI** → `rbk/vov_veiled/RBSFI-foedus_instate.adoc` (Foedus Instate — foedus test-bed contract: rewrite the active-foedus selector RBRR_FOEDUS to a named standing foedus, one field, no self-gate, operator commits. Enchase-adjacent (reuses the feoff/yoke/anoint atomic field-rewrite) but resolves no chained fact, so NOT a rbch_enchase member and rejects in its own precision band. Carries the fixture-composition contract: reuse-if-valid-else-create lives in the fixture composing descry+instate, sharing depot-ensure's combinator — never a fat verb. Operation quoin rbtgo_foedus_instate; word minted, build deferred to the test-infrastructure stream.)
