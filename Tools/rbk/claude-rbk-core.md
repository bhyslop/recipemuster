...
- **RBFD** → `rbk/rbfd*` (Foundry Director Build family — Container (terminal: names no file). Director-side Cloud Build orchestration: `rbfd_ordain`/`rbfd_build`/`rbfd_mirror`/`rbfd_graft` with the reliquary/quota/registry preflights. The `rbfd_director.sh` monolith was decomposed into the children below; the rbfd CLI is now the 0-top `rbfd0_cli.sh`.)
  - **RBFDP** → `rbk/rbfdp_preflight.sh` (Foundry Director preflight — the registry-preflight chain shared by every operation: `zrbfd_registry_preflight`, and beneath it `zrbfd_preflight_reliquary` verifying the reliquary tool cohort; the one shared support the split-worth call-graph survey found)
  - **RBFDB** → `rbk/rbfdb_build.sh` (Foundry Director conjure — `rbfd_build` direct builds.create submission, with the support that partitions to it alone: quota preflight, the builds.create JSON stitcher `zrbfd_stitch_build_json`, and the pouch push `zrbfd_push_build_context`)
  - **RBFDM** → `rbk/rbfdm_mirror.sh` (Foundry Director mirror — `rbfd_mirror` bind-vessel mirror from upstream into GAR via combined Cloud Build, with its submit helper `zrbfd_mirror_submit`)
  - **RBFDG** → `rbk/rbfdg_graft.sh` (Foundry Director graft — `rbfd_graft` local image tag-and-push into GAR under a hallmark minted from the image's own creation timestamp)
  - **RBFDO** → `rbk/rbfdo_ordain.sh` (Foundry Director ordain — the mode dispatcher over the operation bodies, then the metadata pipeline (vouch or graft-metadata submit, cross-module into rbfv) and the hallmark beckon)
  - **RBFD0** → `rbk/rbfd0_director.sh` (Foundry Director 0-trick entry — the module gestalt: single inclusion-guard + tinder + kindle/sentinel. CLI partner `rbfd0_cli.sh`; bare `rbfd_cli.sh` and `rbfd_director.sh` are retired so `rbfd` names no file now that it has children.)
