...
rbhp_federation_entra() {
  ...
  # sections below; the readonly captures survive later buym_*_yawp calls
  ...
  buym_cmd_yawp "«guillemets»"; local -r z_cmd_key_guillemet="${z_buym_yelp}"
  ...
  buym_href_yawp "https://azure.microsoft.com/free" "Azure Free Account Signup"; local -r z_href_azure_free="${z_buym_yelp}"
  ...
  buym_href_yawp "https://entra.microsoft.com" "Microsoft Entra Admin Center"; local -r z_href_entra_center="${z_buym_yelp}"
  buh_line     "   1. Open: ${z_href_entra_center} and sign in as the tenant administrator"
  buym_ui_yawp "Entra ID"; local -r z_ui_entra_id="${z_buym_yelp}"
  buym_ui_yawp "App registrations"; local -r z_ui_app_registrations="${z_buym_yelp}"
  buh_line     "   2. Browse to ${z_ui_entra_id} then ${z_ui_app_registrations}"
  buym_ui_yawp "New registration"; local -r z_ui_new_registration="${z_buym_yelp}"
  buh_line     "   3. Click ${z_ui_new_registration}"
  buh_line     "   4. Configure the registration:"
  buym_cmd_yawp "«your-app-name»"; local -r z_cmd_app_name="${z_buym_yelp}"
  buh_line     "      - Name: ${z_cmd_app_name} (display-only; changeable later)"
  buym_ui_yawp "Supported account types"; local -r z_ui_account_types="${z_buym_yelp}"
  buym_ui_yawp "Single tenant only"; local -r z_ui_single_tenant="${z_buym_yelp}"
  ...
  buym_ui_yawp "Accounts in this organizational directory only"; local -r z_ui_single_tenant_old="${z_buym_yelp}"
  buh_line     "        ${z_ui_single_tenant_old})"
  buym_ui_yawp "Redirect URI"; local -r z_ui_redirect_uri="${z_buym_yelp}"
  ...
  buym_ui_yawp "Register"; local -r z_ui_register="${z_buym_yelp}"
  buh_line     "   5. Click ${z_ui_register}"
  buh_e
  buh_section  "3. Allow the Device Flow (public client):"
  buym_ui_yawp "Manage"; local -r z_ui_manage="${z_buym_yelp}"
  buym_ui_yawp "Authentication"; local -r z_ui_authentication="${z_buym_yelp}"
  buh_line     "   1. On the new registration's page, open ${z_ui_manage} then ${z_ui_authentication}"
  buym_ui_yawp "Advanced settings"; local -r z_ui_advanced="${z_buym_yelp}"
  buym_ui_yawp "Allow public client flows"; local -r z_ui_public_client="${z_buym_yelp}"
  buym_ui_yawp "Yes"; local -r z_ui_yes="${z_buym_yelp}"
  ...
  buym_ui_yawp "Application (client) ID"; local -r z_ui_client_id="${z_buym_yelp}"
  buh_line     "   1. ${z_ui_client_id} — becomes RBRF_IDP_CLIENT_ID below"
  buym_ui_yawp "Directory (tenant) ID"; local -r z_ui_tenant_id="${z_buym_yelp}"
  buym_cmd_yawp "«tenant-id»"; local -r z_cmd_tenant_id="${z_buym_yelp}"
  ...
  buym_ui_yawp "Endpoints"; local -r z_ui_endpoints="${z_buym_yelp}"
  buh_line     "   1. From the registration's ${z_ui_overview} page, click ${z_ui_endpoints}"
  buym_ui_yawp "OpenID Connect metadata document"; local -r z_ui_oidc_metadata="${z_buym_yelp}"
  buh_line     "   2. Open the ${z_ui_oidc_metadata} URL in a browser tab"
  buym_cmd_yawp "issuer"; local -r z_cmd_issuer_field="${z_buym_yelp}"
  buh_line     "   3. Copy the JSON ${z_cmd_issuer_field} value; expect the shape:"
  buym_cmd_yawp "https://login.microsoftonline.com/«tenant-id»/v2.0"; local -r z_cmd_issuer_shape="${z_buym_yelp}"
  ...
  buym_cmd_yawp "RBRF_MECHANISM=rbnfe_interactive"; local -r z_cmd_field_mechanism="${z_buym_yelp}"
  buh_line     "   ${z_cmd_field_mechanism}"
  buym_cmd_yawp "RBRF_IDP_ISSUER=https://login.microsoftonline.com/«tenant-id»/v2.0"; local -r z_cmd_field_issuer="${z_buym_yelp}"
  buh_line     "   ${z_cmd_field_issuer}"
  buh_line     "      (exactly the metadata document's issuer value from step 5)"
  buym_cmd_yawp "RBRF_IDP_CLIENT_ID=«application-client-id»"; local -r z_cmd_field_client="${z_buym_yelp}"
  buh_line     "   ${z_cmd_field_client}"
  buym_cmd_yawp "RBRF_IDP_DEVICE_ENDPOINT=https://login.microsoftonline.com/«tenant-id»/oauth2/v2.0/devicecode"; local -r z_cmd_field_device="${z_buym_yelp}"
  buh_line     "   ${z_cmd_field_device}"
  buym_cmd_yawp "RBRF_IDP_TOKEN_ENDPOINT=https://login.microsoftonline.com/«tenant-id»/oauth2/v2.0/token"; local -r z_cmd_field_token="${z_buym_yelp}"
  ...
  buym_cmd_yawp "RBRF_PROVIDER_ID=«provider-id»"; local -r z_cmd_field_provider="${z_buym_yelp}"
  ...
  buym_ui_yawp "Users"; local -r z_ui_users="${z_buym_yelp}"
  buh_line     "   1. Browse to ${z_ui_entra_id} then ${z_ui_users}; create the user if absent"
  buym_ui_yawp "Object ID"; local -r z_ui_object_id="${z_buym_yelp}"
  ...
  buym_ui_yawp "API permissions"; local -r z_ui_api_permissions="${z_buym_yelp}"
