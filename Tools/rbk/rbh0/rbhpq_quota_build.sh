...
rbhp_quota_build() {
  ...
  buym_cmd_yawp "(2 vCPU)   → 5 concurrent builds"; local -r z_cmd_concur_2vcpu="${z_buym_yelp}"
  buh_line     "     e2-standard-2  ${z_cmd_concur_2vcpu}"
  buym_cmd_yawp "(8 vCPU)   → 1 concurrent build"; local -r z_cmd_concur_8vcpu="${z_buym_yelp}"
  buh_line     "     e2-standard-8  ${z_cmd_concur_8vcpu}"
  buym_cmd_yawp "(32 vCPU)  → needs 32+ CPU quota"; local -r z_cmd_concur_32vcpu="${z_buym_yelp}"
  buh_line     "     e2-standard-32 ${z_cmd_concur_32vcpu}"
  buh_e
  buh_section  "Key:"
  buym_ui_yawp "precise words you see on the web page."; local -r z_ui_key_magenta="${z_buym_yelp}"
  buh_line     "   Magenta text refers to ${z_ui_key_magenta}"
  buym_cmd_yawp "something you might copy from here."; local -r z_cmd_key_cyan="${z_buym_yelp}"
  ...
  buym_cmd_yawp "${RBDC_DEPOT_PROJECT_ID}"; local -r z_cmd_regime_depot_project="${z_buym_yelp}"
  buh_line     "   RBDC_DEPOT_PROJECT_ID:          ${z_cmd_regime_depot_project}"
  buym_cmd_yawp "${RBRD_GCP_REGION}"; local -r z_cmd_regime_gcp_region="${z_buym_yelp}"
  buh_line     "   RBRD_GCP_REGION:                ${z_cmd_regime_gcp_region}"
  buym_cmd_yawp "${RBRD_GCB_MACHINE_TYPE}"; local -r z_cmd_regime_machine_type="${z_buym_yelp}"
  buh_line     "   RBRD_GCB_MACHINE_TYPE:          ${z_cmd_regime_machine_type}"
  buym_cmd_yawp "${RBDC_GCB_POOL_STEM}"; local -r z_cmd_regime_pool_stem="${z_buym_yelp}"
  buh_line     "   RBDC_GCB_POOL_STEM:             ${z_cmd_regime_pool_stem}"
  buym_cmd_yawp "${RBRR_GCB_MIN_CONCURRENT_BUILDS}"; local -r z_cmd_regime_min_concurrent="${z_buym_yelp}"
  ...
  buh_e
  buym_href_yawp "https://console.cloud.google.com/iam-admin/quotas?project=${RBDC_DEPOT_PROJECT_ID}" "Quotas and System Limits (opens to depot project)"; local -r z_href_check_quotas="${z_buym_yelp}"
  buh_line     "   Go to: ${z_href_check_quotas}"
  buym_ui_yawp "${RBDC_DEPOT_PROJECT_ID}"; local -r z_ui_check_project="${z_buym_yelp}"
  buh_line     "   1. Verify project ${z_ui_check_project} is selected in the project picker"
  buym_ui_yawp "Enter property name or value"; local -r z_ui_check_filter_bar="${z_buym_yelp}"
  buh_line     "   2. In the ${z_ui_check_filter_bar} filter bar, type:"
  buym_cmd_yawp "concurrent_private"; local -r z_cmd_check_filter_text="${z_buym_yelp}"
  buh_line     "      ${z_cmd_check_filter_text}"
  buym_ui_yawp "cloudbuild.googleapis.com/concurrent_private_pool_build_cpus"; local -r z_ui_check_autocomplete="${z_buym_yelp}"
  ...
  buym_ui_yawp "Quota"; local -r z_ui_check_type_quota="${z_buym_yelp}"
  ...
  buym_ui_yawp "Quota"; local -r z_ui_request_row_quota="${z_buym_yelp}"
  buh_line     "   On the ${z_ui_request_row_quota} row identified above:"
  buym_ui_yawp "⋮"; local -r z_ui_request_menu_dots="${z_buym_yelp}"
  buh_line     "   1. Click the three-dot menu ${z_ui_request_menu_dots} at the right end of the row"
  buym_ui_yawp "Edit quota"; local -r z_ui_request_edit_quota="${z_buym_yelp}"
  buh_line     "   2. Select ${z_ui_request_edit_quota}"
  buym_ui_yawp "New value"; local -r z_ui_request_new_value="${z_buym_yelp}"
  ...
  buym_ui_yawp "Request description"; local -r z_ui_request_description="${z_buym_yelp}"
  buh_line     "   4. A ${z_ui_request_description} field appears. Enter:"
  buym_cmd_yawp "Need parallel builds on private worker pool for CI/CD pipeline testing."; local -r z_cmd_request_description_text="${z_buym_yelp}"
  buh_line     "      ${z_cmd_request_description_text}"
  buym_ui_yawp "Next"; local -r z_ui_request_next="${z_buym_yelp}"
  buh_line     "   5. Click ${z_ui_request_next}"
  buh_line     "   6. Step 2/2 shows contact details (pre-filled from your Google account)"
  buym_ui_yawp "Submit request"; local -r z_ui_request_submit="${z_buym_yelp}"
  ...
  buym_href_yawp "https://console.cloud.google.com/iam-admin/quotas?project=${RBDC_DEPOT_PROJECT_ID}" "Quotas and System Limits (opens to depot project)"; local -r z_href_confirm_quotas="${z_buym_yelp}"
  buh_line     "   Return to: ${z_href_confirm_quotas}"
  buym_ui_yawp "concurrent_private"; local -r z_ui_confirm_filter_text="${z_buym_yelp}"
  ...
  buym_cmd_yawp "${RBRR_GCB_MIN_CONCURRENT_BUILDS} concurrent builds"; local -r z_cmd_confirm_target="${z_buym_yelp}"
