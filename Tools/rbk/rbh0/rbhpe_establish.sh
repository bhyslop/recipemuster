...
rbhp_establish() {
  ...
  # buym_cmd_yawp calls overwriting the shared z_buym_yelp scratch.
  buym_cmd_yawp "${RBGC_PAYOR_APP_NAME}"; local -r z_cmd_app_name="${z_buym_yelp}"
  ...
  buh_e
  buh_section  "Key:"
  buym_ui_yawp "precise words you see on the web page."; local -r z_ui_key_words="${z_buym_yelp}"
  buh_line     "   Magenta text refers to ${z_ui_key_words}"
  buym_cmd_yawp "something you might copy from here."; local -r z_cmd_key_copy="${z_buym_yelp}"
  buh_line     "   Cyan text is ${z_cmd_key_copy}"
  buym_href_yawp "https://example.com/" "EXAMPLE DOT COM"; local -r z_href_key_example="${z_buym_yelp}"
  ...
  buym_cmd_yawp "${ZRBHP_RBRP_FILE}"; local -r z_cmd_rbrp_file="${z_buym_yelp}"
  buh_line     "   File: ${z_cmd_rbrp_file}"
  buym_cmd_yawp "${RBRP_PAYOR_PROJECT_ID}"; local -r z_cmd_payor_project_id="${z_buym_yelp}"
  ...
  buym_href_yawp "https://console.cloud.google.com/cloud-resource-manager" "Google Cloud Project List"; local -r z_href_project_list="${z_buym_yelp}"
  buh_line     "   1. Check existing projects: ${z_href_project_list}"
  buym_ui_yawp "${RBRP_PAYOR_PROJECT_ID}"; local -r z_ui_payor_project_id="${z_buym_yelp}"
  ...
  buym_cmd_yawp "find the project"; local -r z_cmd_find_project="${z_buym_yelp}"
  buym_ui_yawp "${ZRBHP_RBRP_FILE_BASENAME}"; local -r z_ui_rbrp_file_basename="${z_buym_yelp}"
  ...
  buym_href_yawp "https://console.cloud.google.com/projectcreate" "Google Cloud Project Create"; local -r z_href_project_create="${z_buym_yelp}"
  ...
  buym_ui_yawp "Organization"; local -r z_ui_organization="${z_buym_yelp}"
  buym_ui_yawp "No organization"; local -r z_ui_no_organization="${z_buym_yelp}"
  buh_line     "      - Location: your ${z_ui_organization} (the one founded in the organization"
  buh_line     "        guide) — NOT ${z_ui_no_organization}. The consent screen's Internal"
  buh_line     "        audience (step 6) is only offered on an org-hosted project, and the"
  buh_line     "        manor-setup finisher gates on it."
  buym_ui_yawp "Edit"; local -r z_ui_edit="${z_buym_yelp}"
  buh_line     "   4. Click ${z_ui_edit} next to the auto-generated Project ID"
  buym_cmd_yawp "${RBRP_PAYOR_PROJECT_ID}"; local -r z_cmd_payor_project_id_edit="${z_buym_yelp}"
  buh_line     "   5. Replace it with: ${z_cmd_payor_project_id_edit}"
  buym_ui_yawp "CREATE"; local -r z_ui_create_project="${z_buym_yelp}"
  buh_line     "   6. Click ${z_ui_create_project}"
  buym_ui_yawp "Creating project..."; local -r z_ui_creating_project="${z_buym_yelp}"
  ...
  buym_href_yawp "https://console.cloud.google.com/apis/dashboard?project=${RBRP_PAYOR_PROJECT_ID}" "Google Cloud APIs Dashboard"; local -r z_href_apis_dashboard_verify="${z_buym_yelp}"
  buh_line     "   1. Test this link: ${z_href_apis_dashboard_verify}"
  buym_ui_yawp "Project Picker"; local -r z_ui_project_picker="${z_buym_yelp}"
  buh_line     "   2. The page is correct when the ${z_ui_project_picker} button reads ${z_cmd_app_name}"
  buym_ui_yawp "You need additional access"; local -r z_ui_need_additional_access="${z_buym_yelp}"
  buh_line     "   3. If you see ${z_ui_need_additional_access}, wait a few minutes and refresh the page"
  buym_href_yawp "https://cloud.google.com/iam/docs/access-change-propagation" "Access Change Propagation"; local -r z_href_access_propagation="${z_buym_yelp}"
  buh_line     "      GCP IAM changes are eventually consistent: ${z_href_access_propagation}"
  buh_e
  buh_section  "5. Configure Billing Account:"
  buym_href_yawp "https://console.cloud.google.com/billing" "Google Cloud Billing"; local -r z_href_billing="${z_buym_yelp}"
  buh_line     "   1. Go to: ${z_href_billing}"
  buh_line     "      If no billing accounts exist:"
  buym_ui_yawp "Create account"; local -r z_ui_create_account="${z_buym_yelp}"
  buh_line     "          a. Click ${z_ui_create_account}"
  buh_line     "          b. Configure payment method and submit"
  buym_ui_yawp "Account ID"; local -r z_ui_account_id_new="${z_buym_yelp}"
  buh_line     "          c. Copy new ${z_ui_account_id_new} from table"
  buh_line     "      else if single Open account exists:"
  buym_ui_yawp "Account ID"; local -r z_ui_account_id_single="${z_buym_yelp}"
  ...
  buym_ui_yawp "Account ID"; local -r z_ui_account_id_chosen="${z_buym_yelp}"
  buh_line     "          b. Copy chosen ${z_ui_account_id_chosen} value"
  buym_href_yawp "https://console.cloud.google.com/billing/projects" "Google Cloud Billing Projects"; local -r z_href_billing_projects="${z_buym_yelp}"
  buh_line     "   2. Go to: ${z_href_billing_projects}"
  buym_ui_yawp "${ZRBHP_RBRP_FILE}"; local -r z_ui_rbrp_file_save="${z_buym_yelp}"
  buh_line     "   3. Save the billing account ID to your ${z_ui_rbrp_file_save}"
  buym_cmd_yawp "RBRP_BILLING_ACCOUNT_ID="; local -r z_cmd_billing_account_id="${z_buym_yelp}"
  buym_ui_yawp "Value from Account ID column"; local -r z_ui_account_id_value="${z_buym_yelp}"
  ...
  buym_ui_yawp "${ZRBHP_RBRP_FILE}"; local -r z_ui_rbrp_file_update="${z_buym_yelp}"
  buh_line     "   5. Update ${z_ui_rbrp_file_update} and re-display this procedure."
  buh_e
  buh_section  "6. Configure OAuth Consent Screen:"
  buym_href_yawp "https://console.cloud.google.com/apis/credentials/consent?project=${RBRP_PAYOR_PROJECT_ID}" "OAuth consent screen"; local -r z_href_oauth_consent="${z_buym_yelp}"
  buh_line     "   Go to: ${z_href_oauth_consent}"
  buym_ui_yawp "Google Auth Platform not configured yet"; local -r z_ui_auth_not_configured="${z_buym_yelp}"
  buh_line     "   1. The console displays ${z_ui_auth_not_configured}"
  buym_ui_yawp "Get started"; local -r z_ui_get_started="${z_buym_yelp}"
  ...
  buym_ui_yawp "Next"; local -r z_ui_next_step1="${z_buym_yelp}"
  buh_line     "        - Click ${z_ui_next_step1}"
  buh_line     "      Step 2 - Audience:"
  buym_ui_yawp "Internal"; local -r z_ui_internal="${z_buym_yelp}"
  ...
  buym_ui_yawp "Next"; local -r z_ui_next_step2="${z_buym_yelp}"
  buh_line     "        - Click ${z_ui_next_step2}"
  buh_line     "      Step 3 - Contact Information:"
  buh_line     "        - Email addresses: (your email), press Enter"
  buym_ui_yawp "Next"; local -r z_ui_next_step3="${z_buym_yelp}"
  buh_line     "        - Click ${z_ui_next_step3}"
  buh_line     "      Step 4 - Finish:"
  buym_ui_yawp "I agree to the Google API Services: User Data Policy"; local -r z_ui_agree_policy="${z_buym_yelp}"
  buh_line     "        - Read the linked terms, then (if you accept) check ${z_ui_agree_policy}"
  buym_ui_yawp "Continue"; local -r z_ui_continue="${z_buym_yelp}"
  buh_line     "        - Click ${z_ui_continue}"
  buym_ui_yawp "Create"; local -r z_ui_create_consent="${z_buym_yelp}"
  buh_line     "        - Click ${z_ui_create_consent}"
  buh_e
  buh_section  "7. Create OAuth 2.0 Client ID:"
  buym_href_yawp "https://console.cloud.google.com/apis/credentials?project=${RBRP_PAYOR_PROJECT_ID}" "Credentials"; local -r z_href_credentials="${z_buym_yelp}"
  buh_line     "   Go to: ${z_href_credentials}"
  buym_ui_yawp "+ Create credentials"; local -r z_ui_create_credentials="${z_buym_yelp}"
  buh_line     "   1. From top bar, click ${z_ui_create_credentials}"
  buym_ui_yawp "OAuth client ID"; local -r z_ui_oauth_client_id="${z_buym_yelp}"
  buh_line     "   2. Select ${z_ui_oauth_client_id}"
  buym_ui_yawp "Desktop app"; local -r z_ui_desktop_app="${z_buym_yelp}"
  buh_line     "   3. Application type: ${z_ui_desktop_app}"
  buh_line     "   4. Name: ${z_cmd_app_name}"
  buym_ui_yawp "Create"; local -r z_ui_create_client="${z_buym_yelp}"
  buh_line     "   5. Click ${z_ui_create_client}"
  buym_ui_yawp "OAuth client created"; local -r z_ui_oauth_client_created="${z_buym_yelp}"
  buh_line     "   6. Popup titled ${z_ui_oauth_client_created} displays client ID and secret"
  buym_ui_yawp "Download JSON"; local -r z_ui_download_json="${z_buym_yelp}"
  buh_line     "   7. Click ${z_ui_download_json}"
  buym_ui_yawp "OK"; local -r z_ui_ok="${z_buym_yelp}"
  buym_ui_yawp "client_secret_[id].apps.googleusercontent.com.json"; local -r z_ui_client_secret_filename="${z_buym_yelp}"
  buh_line     "   8. Click ${z_ui_ok} ; browser downloads ${z_ui_client_secret_filename}"
  buym_warn_yawp "CRITICAL: Save securely - contains client secret"; local -r z_warn_save_securely="${z_buym_yelp}"
