...
zjjw_emit_notch_command() {
  {
    ...
    echo ""
    echo "5. Check for new (untracked) files:"
    echo "   \`\`\`bash"
    ...
    echo ""
    echo "6. Dispatch to notcher agent:"
    echo "   - Use Task tool with subagent_type='general-purpose' and model='haiku'"
    echo "   - Prompt template (substitute actual values for HEAT, PACE, BRAND):"
    echo "     \`\`\`"
    echo "     Git commit agent. Heat=HEAT, Pace=PACE, Brand=BRAND"
    echo "     Format:"
    echo "       Line 1: [jj:BRAND][HEAT/PACE] Summary (imperative, under 72 chars)"
    echo "       Line 2: blank"
    ...
    echo "     \`\`\`"
    echo ""
    echo "7. Report result from agent (commit hash or failure)"
    echo ""
    ...
    echo "   \`\`\`"
    echo ""
    echo "9. Re-engage with current pace:"
