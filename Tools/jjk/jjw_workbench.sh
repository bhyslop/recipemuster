...
zjjw_emit_notch_command() {
  local z_brand="${1}"
  {
    ...
    echo ""
    echo "3. Extract current pace from context"
    echo "   - The current pace should be clear from conversation (bolded item in heat's ## Remaining)"
    echo "   - Extract pace silks from title: \`**Fix quota bug**\` → \`fix-quota-bug\`"
    echo "   - If no pace context: fail with \"No pace context - run /jja-heat-saddle first\""
    echo ""
    ...
    echo ""
    echo "5. Dispatch to notcher agent:"
    echo "   - Use Task tool with subagent_type='general-purpose' and model='haiku'"
    echo "   - Prompt template (substitute HEAT and PACE at runtime):"
    echo "     \`\`\`"
    echo "     Git commit agent. Heat=HEAT, Pace=PACE, Brand=${z_brand}"
    echo "     Format:"
    echo "       Line 1: [jj:${z_brand}][HEAT/PACE] Summary (imperative, under 72 chars)"
    echo "       Line 2: blank"
    echo "       Lines 3+: - Detail bullet per logical change"
    echo "     Process: git add -u, git diff --cached, write message, git commit -m \"...\", report hash."
    echo "     \`\`\`"
    echo ""
    echo "6. Report result from agent (commit hash or failure)"
    echo ""
    ...
    echo "   \`\`\`"
    echo ""
    echo "8. Re-engage with current pace:"
    echo "   - If there is a current pace (first in ## Remaining):"
    echo "     - Read the pace description and any files/context it references"
    echo "     - Analyze what the work entails"
    echo "     - Propose a concrete approach (2-4 bullets)"
    echo "     - Ask: \"Ready to proceed with this approach?\""
    echo "     - On approval: begin work directly"
    echo "   - If no current pace:"
    echo "     - Announce \"All paces complete - ready to retire heat?\""
    echo ""
    echo "Error handling: If heat/pace context missing or ambiguous, fail immediately - do not attempt recovery."
  }
}
