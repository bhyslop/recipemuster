...
#   jjk-u  Uninstall Job Jockey (preserves .claude/jji/ state)
...
ZJJW_KIT_PATH="${ZJJW_KIT_PATH:-Tools/jjk/job-jockey-kit.md}"
...
zjjw_emit_heat_resume() {
  {
    ...
    echo "Steps:"
    echo ""
    echo "1. Check for heat files in .claude/jji/current/"
    echo "   - Look for files matching pattern \`jjh-b*.md\`"
    echo ""
    echo "2. Branch based on heat count:"
    echo ""
    echo "   **If 0 heats:**"
    echo "   - Check .claude/jji/pending/ for pending heats"
    echo "   - Announce: \"No active heat found\""
    echo "   - If pending heats exist: \"You have N pending heat(s): [list names]\""
    echo "   - Ask: \"Would you like to start a new heat, activate a pending heat, or promote an itch?\""
    ...
    echo "   \`\`\`"
    echo ""
    echo "Error handling: If .claude/jji/current/ doesn't exist, announce issue and stop."
  }
}
...
zjjw_emit_heat_retire() {
  {
    ...
    echo "   - Current filename pattern: \`jjh-bYYMMDD-description.md\`"
    echo "   - Extract the begin date (bYYMMDD) and description"
    echo "   - Generate retire date: today's date as rYYMMDD"
    echo "   - New filename: \`jjh-bYYMMDD-rYYMMDD-description.md\`"
    echo ""
    echo "   Example:"
    echo "   - Before: \`jjh-b251108-buk-rename.md\`"
    echo "   - After: \`jjh-b251108-r251127-buk-rename.md\`"
    echo ""
    echo "4. Move the file:"
    echo "   \`\`\`bash"
    echo "   git mv .claude/jji/current/jjh-b251108-buk-rename.md .claude/jji/retired/jjh-b251108-r251127-buk-rename.md"
    echo "   \`\`\`"
    echo ""
    ...
    echo "   - File: .claude/jji/retired/jjh-b251108-r251127-buk-rename.md"
    echo "   \`\`\`"
    echo ""
    echo "7. Offer next steps:"
    echo "   - \"Would you like to start a new heat or check jjf-future.md for itches to promote?\""
    echo ""
...
zjjw_emit_pace_refine() {
  {
    ...
    echo "Steps:"
    echo ""
    echo "1. Check for current heat in .claude/jji/current/"
    echo "   - If no heat: announce \"No active heat\" and stop"
    echo "   - If multiple: ask which one"
    echo ""
    echo "2. Ask which pace to refine (or infer from context)"
    echo ""
    echo "3. Read the current pace spec and assess its state:"
    echo "   - Is mode defined? (manual/delegated/unset)"
    echo "   - Is spec sparse or detailed?"
    echo ""
    echo "4. Conduct adaptive interview:"
    echo ""
    echo "   If spec is sparse/new:"
    echo "   - \"Is this a manual pace (you drive) or should we prepare it for delegation (model drives)?\""
    echo "   - If manual: confirm and done"
    echo "   - If delegated: continue to step 5"
    echo ""
    echo "   If spec exists:"
    echo "   - Show current spec summary"
    echo "   - \"What needs to change?\""
    echo "   - Focus on the delta"
    echo ""
    echo "5. For delegated mode, ensure spec covers:"
    ...
    echo ""
    echo "   Ask only for missing elements."
    echo ""
...
zjjw_emit_pace_wrap() {
  {
    ...
    echo ""
    echo "5. Update the heat file in .claude/jji/current/:"
    ...
    echo "   \`\`\`bash"
    echo "   git add .claude/jji/current/jjh-*.md"
...
zjjw_emit_itch_list() {
  {
    echo "You are listing all Job Jockey itches."
    echo ""
    echo "Configuration:"
    echo "- Target repo dir: ${ZJJW_TARGET_DIR}"
    echo "- Kit path: ${ZJJW_KIT_PATH}"
    ...
    echo "Steps:"
    echo ""
    echo "1. Read .claude/jji/jjf-future.md for future itches"
    echo ""
    echo "2. Read .claude/jji/jjs-shelved.md for shelved itches"
    echo ""
    echo "3. Display all itches:"
    echo "   \`\`\`"
    echo "   Future itches (N):"
    echo "   1. [itch title/description]"
    echo "   2. [itch title/description]"
    echo "   ..."
    echo ""
    echo "   Shelved itches (N):"
    echo "   1. [itch title/description] - [reason if noted]"
    echo "   2. [itch title/description]"
    echo "   ..."
    echo "   \`\`\`"
    echo ""
    echo "4. If either file is empty or missing, report appropriately:"
    echo "   - \"Future itches: none\""
    echo "   - \"Shelved itches: none\""
    echo ""
...
zjjw_emit_itch_find() {
  {
    ...
    echo "   - .claude/jji/jjf-future.md"
    echo "   - .claude/jji/jjs-shelved.md"
    echo ""
    ...
    echo ""
    echo "   In Future:"
    echo "   - [matching itch with surrounding context]"
    echo ""
    echo "   In Shelved:"
    echo "   - [matching itch with surrounding context]"
    echo "   \`\`\`"
    echo ""
    echo "4. If no matches found, report \"No itches found matching '[term]'\""
    echo ""
...
zjjw_emit_itch_move() {
  {
    ...
    echo "   - .claude/jji/jjf-future.md (Future)"
    echo "   - .claude/jji/jjs-shelved.md (Shelved)"
    echo ""
    echo "3. Ask for destination:"
    echo "   - Future (jjf-future.md) - worthy of doing"
    echo "   - Shelved (jjs-shelved.md) - setting aside"
    echo "   - Promote to Heat - create new jjh-*.md file"
    echo ""
    echo "4. Execute the move:"
    echo ""
    echo "   **If moving between Future/Shelved:**"
    echo "   - Remove from source file"
    echo "   - Add to destination file"
    echo "   - Preserve any context/notes"
    echo ""
    echo "   **If promoting to Heat:**"
    echo "   - Remove from source file"
    echo "   - Create new heat file: .claude/jji/current/jjh-bYYMMDD-[description].md"
    ...
    echo ""
    echo "Error handling: If itch not found or files missing, announce issue and stop."
  }
}
...
zjjw_emit_claudemd_section() {
  {
    ...
    echo ""
    echo "**Concepts:**"
    echo "- **Heat**: Bounded initiative (3-50 sessions), has paces. Location indicates state: \`current/\` (active), \`pending/\` (parked), \`retired/\` (done). Move to \`current/\` via prose when ready to work. Park in \`pending/\` via prose when blocked or deferring."
    echo "- **Pace**: Discrete action within a heat; mode is \`manual\` (human drives) or \`delegated\` (model drives from spec)"
    echo "- **Itch**: Future idea, lives in Future or Shelved"
    echo ""
    echo "- Target repo dir: \`${ZJJW_TARGET_DIR}\`"
    ...
    echo "- \`/jja-itch-list\` - List all itches (future and shelved)"
    echo "- \`/jja-itch-find\` - Find an itch by keyword"
    echo "- \`/jja-itch-move\` - Move or promote an itch"
    echo ""
...
jjw_install() {
  ...
  mkdir -p ".claude/jji/current"
  mkdir -p ".claude/jji/pending"
  mkdir -p ".claude/jji/retired"
  test -f ".claude/jji/jjf-future.md" || echo "# Future Itches" > ".claude/jji/jjf-future.md"
  test -f ".claude/jji/jjs-shelved.md" || echo "# Shelved Itches" > ".claude/jji/jjs-shelved.md"
...
jjw_uninstall() {
  ...
  buc_success "Job Jockey uninstalled (state preserved in .claude/jji/)"
}
