...
use crate::jjrmt_mount::{jjrmt_MountArgs, jjrmt_run_mount};
...
/// `jjrm_mcp` alias, so the literal is never re-spelled here.
fn gallops_pathbuf() -> PathBuf {
    crate::jjrc_core::jjrc_default_gallops_path()
}
...
pub(crate) fn zjjrm_studbook_config() -> Result<jjdb_BlotterConfig, String> {
    ...
    Ok(jjdb_studbook_config(&infield_root))
}
...
pub(crate) fn zjjrm_load_gallops(path: &Path) -> Result<jjrg_Gallops, String> {
    ...
    let studbook = zjjrm_studbook_config()?;
...
pub(crate) fn zjjrm_studbook_root() -> Result<PathBuf, String> {
    Ok(zjjrm_studbook_config()?.local_root)
}
...
/// OF RECORD. `jjx_validate` (`jjrvl_validate.rs`) does NOT write through this
/// funnel, but it is no longer exempt from the studbook — it carries its own
/// const-gated `_over` seam (`jjrvl_run_validate_over`), because its I/O shape is
/// unlike a mutating command's: it reads the STORED BYTES for a byte-canonical
/// comparison (which this funnel's parsed read discards) and only conditionally
/// writes (on a non-canonical store). Seam-on it appraises the studbook tip and
/// journals any normalization back through `zjjrm_journal_run` — the banked
/// flip-time question (name the studbook, or notice-and-noop against the fossil
/// consumer gallops) resolved to name the studbook, so `jjx_validate` checks the
/// store of record. In practice that normalize is a near-dead safety net — every
/// studbook write lands canonical through `jjdr_save` and the reprieve
/// write-forward rides `jjdr_hark` on the next read — reachable only by a live
/// reprieve migration, a serializer drift across binary versions, or a hand-edited
/// tip. The merge-finalization duty the old exemption named is retired: the
/// studbook is a linear blotter that never merges. `jjri_consign` (the consumer
/// self-commit) survives only as validate's seam-OFF write, exercised by the
/// byte-fixture tests.
#[allow(clippy::too_many_arguments)]
...
/// paneboard default — the writer simply omits the attribute. Sibling to the
/// consumer Gallops path under `.claude/jjm/`; the `jje_` file prefix is its home.
const EMBLEM_STYLE_PATH: &str = ".claude/jjm/jje_emblem.json";

/// Officium-resident marker recording what work identity this session last
/// resolved onto (mount or groom): the identity plus its resolved silks.
/// Written by the mount and groom arms — orient records the mounted pace's
/// coronet (the resolved next-actionable one, never the bare firemark lede)
/// with its pace+heat silks, show records the heat firemark with heat silks
/// only — and read by the emblem writer so every later jjx engagement —
/// `record`, `list`, `close`, … — paints that identity, not just orient/show
/// itself. Which identity is JJK's mount/groom semantics, not the agent's lede
/// choice (paddock "Emblem and window reference"). The silks are resolved once
/// at mount/groom and cached here, so the per-engagement writer reads them
/// without a gallops touch (they refresh on the next mount/groom, the same
/// staleness window as the identity itself). Absent before the first mount:
/// the emblem then degrades to the bare officium identity (see
/// `zjjrm_refresh_emblem`). Lives under the gitignored officia tree, alongside
/// `heartbeat`.
const EMBLEM_MARKER_FILE: &str = "emblem";

/// The emblem marker's on-disk shape (JSON under `EMBLEM_MARKER_FILE`). Carries
...
/// Resolve the emblem marker — identity plus its silks — for a freshly mounted
...
/// True when this officium's standing emblem marker already holds a *coronet*
/// identity — the coronet-sticks guard (JJS0 `jjdxw_emblem`): once a mount has
/// resolved onto a coronet, a later groom must not demote it to the heat
...
/// is fillable, so the groom resolves normally. Officium-scoped, like the marker
/// itself.
fn zjjrm_standing_is_coronet(officium_dir: &Path) -> bool {
    std::fs::read_to_string(officium_dir.join(EMBLEM_MARKER_FILE))
        .ok()
        .and_then(|s| serde_json::from_str::<jjrm_EmblemMarker>(&s).ok())
...
///
/// `new_marker` is the freshly-resolved emblem marker to persist. The mounting
...
/// officium's `EMBLEM_MARKER_FILE`; the emblem is then composed from whatever
/// marker stands there, degrading to the bare officium handle (the `☉` session
/// id) with no silks when no mount has happened yet.
fn zjjrm_refresh_emblem(officium_dir: &Path, new_marker: Option<jjrm_EmblemMarker>) {
    ...
    if let Some(ref m) = new_marker {
        if let Ok(json) = serde_json::to_string(m) {
            let _ = std::fs::write(officium_dir.join(EMBLEM_MARKER_FILE), json.as_bytes());
        }
    }
    ...
    let marker: Option<jjrm_EmblemMarker> =
        std::fs::read_to_string(officium_dir.join(EMBLEM_MARKER_FILE))
            .ok()
            .and_then(|s| serde_json::from_str(&s).ok());
...
/// The explicit-config resolver — the ONE path composition for an officium's
/// exchange dir, driven by tests with a fixture studbook config and by the
/// live wrapper with the process-lifetime config below. The former
/// `over_studbook` flag and its pre-cutover relative-join arm are gone with
/// the cutover: there is no fork left for a wrapper and a testable branch to
/// disagree across.
/// `jjrlg_legatio.rs`'s legatio/pensum state resolves through the wrapper
/// over this SAME function (no separate copy), so it relocates in lockstep
/// with gazettes.
pub(crate) fn zjjrm_exchange_dir_over(
    ...
    let bare_id = officium.trim_start_matches(OFFICIUM_SUN_PREFIX);
    jjrm_studbook_exchange_dir(&studbook.local_root, bare_id)
}

/// The studbook config behind the officium root, resolved ONCE per server
/// process from the launch cwd and stable for the process lifetime. The
/// officium root must never move under a running server — gazette paths are
...
fn zjjrm_officium_studbook() -> &'static jjdb_BlotterConfig {
    ZJJRM_OFFICIUM_STUDBOOK.get_or_init(|| {
        let cwd = std::env::current_dir().expect("officium root requires a readable cwd");
        let infield_root = zjjrm_infield_root(&jjrfg_PlainGit, &cwd)
            .expect("officium root requires a resolvable infield root from the server's launch cwd");
        jjdb_studbook_config(&infield_root)
    })
}
...
/// duplicated path join, so that state relocates in lockstep with gazettes.
///
/// The emitted path is absolute and the agent uses it verbatim, never
/// reconstructing it from the id — reconstruction is exactly the trap the
/// ☉-glyph strip set: the id carries the glyph, the on-disk dir does not,
/// and a hand-built path lands in a sibling that does not exist.
/// Resolution rides the process-lifetime officium studbook
/// (`zjjrm_officium_studbook`), never a per-call ambient-cwd read.
pub fn jjrm_exchange_dir(officium: &str) -> std::path::PathBuf {
    zjjrm_exchange_dir_over(officium, zjjrm_officium_studbook())
}

/// The officia root — the parent directory holding every officium's
/// exchange dir, needed before any one officium's id is minted
/// (`zjjrm_handle_open`'s own list-and-mint loop). Rides the same
/// process-lifetime officium studbook as `jjrm_exchange_dir`.
fn zjjrm_officia_root() -> PathBuf {
    zjjrm_officium_studbook()
        .local_root
...
/// (`jjrvb_blotter.rs`). Flipped live (`true`) by ₣B3's founding-and-cutover
/// ceremony (JJSAS Founding-and-cutover). Its one remaining branch site is
/// the station-name refusal in `zjjrm_handle_open` — the exchange-dir
/// resolvers no longer fork on it: post-cutover their pre-seam arms were
/// dead code and are gone, every resolution riding the process-lifetime
/// officium studbook. Nothing outside this module reads the constant
/// directly — every other module (including `jjrlg_legatio.rs`) reaches the
/// officium exchange only by calling `jjrm_exchange_dir`, the sole funnel
/// every officium-exchange-dir construction converges onto.
...
/// Best-effort studbook tip read for the lockless open-time monita — the
/// studbook-of-record analogue of their former raw `std::fs::read` of the
/// consumer file (which post-cutover is a tombstone that never parses, so the
/// monita silently died at the flip). Returns the parsed Gallops beside the raw
/// tip bytes (the reprieve probe needs both), or `None` on any failure — the
/// monita are non-gating by contract, so a stale or absent clone must fold to
/// silence, never block open. Reads the local pinned tip only (the studbook
/// glean already ran earlier in open), never a network fetch. Takes the studbook
/// config so a test can aim it at a fixture store; the monita derive theirs from
/// cwd through the one `zjjrm_studbook_config` resolver.
pub(crate) fn zjjrm_peek_gallops(studbook: &jjdb_BlotterConfig) -> Option<(jjrg_Gallops, Vec<u8>)> {
    let pin = jjdb_pin(studbook).ok()?;
    let bytes = jjdb_read_pinned(studbook, &pin, JJDB_GALLOPS_REL_PATH).ok()?;
    let gallops: jjrg_Gallops = serde_json::from_slice(&bytes).ok()?;
    Some((gallops, bytes))
}

/// Best-effort, read-only reprieve nag for jjx_open.
///
/// Peeks the studbook tip without the commit lock and emits one status line per registered
...
/// succeeds. The lockless peek reads the pinned tip's object bytes and mutates nothing.
fn zjjrm_reprieve_nag(output: &mut vvc::vvco_Output) {
    let studbook = match zjjrm_studbook_config() {
        Ok(s) => s,
        Err(_) => return,
    };
    let (gallops, bytes) = match zjjrm_peek_gallops(&studbook) {
        Some(g) => g,
        None => return,
    };
...
///
/// Peeks the studbook tip without the commit lock and reports the chat-history retention policy
...
/// a malformed date is a read-time classification, not a parse error. The lockless peek reads the
/// pinned tip's object bytes and mutates nothing.
fn zjjrm_retention_monitum(output: &mut vvc::vvco_Output) {
    let studbook = match zjjrm_studbook_config() {
        Ok(s) => s,
        Err(_) => return,
    };
    let (gallops, _bytes) = match zjjrm_peek_gallops(&studbook) {
        Some(g) => g,
        None => return,
    };
...
async fn zjjrm_handle_open(size_limit: u64) -> Result<CallToolResult, ErrorData> {
    ...
    // Degraded emblem paint: a freshly-opened officium has no resolved identity
    ...
    //
    // Const-gated OFF post-cutover: the store of record is the studbook, so there is no consumer
    // gallops to converge, and saving one back to the in-repo path would resurrect the Gallops
    // shape its tombstone (JJr_d9f) forbids ("Do not restore Gallops shape to this file"). The
...
impl jjrm_McpServer {
    ...
    async fn jjx(&self, Parameters(p): Parameters<jjrm_JjxParams>) -> Result<CallToolResult, ErrorData> {
        ...
        // session last resolved onto. The mounting and grooming commands (orient,
        // show) re-derive and re-record the correct identity from their OWN
        ...
        match cmd {
            ...
            JJRM_CMD_NAME_ORIENT => {
                ...
                // Require the gazette halter notice; orient mounts exactly one target.
                ...
                if targets.len() != 1 {
                    return Ok(CallToolResult::error(vec![Content::text(format!(
                        "{}: orient mounts one target — expected exactly one {} notice, got {}", cmd, JJRZ_SLUG_HALTER, targets.len(),
                    ))]));
                }
                ...
                if code == 0 {
                    // Record the identity this mount actually landed on, derived
                    // from the mount routine's own resolution rather than the
                    // halter lede: the resolved next-actionable coronet — which
                    // it emitted as the Pace notice whether the lede was a
                    // firemark (next actionable pace) or a coronet (looked up
                    // directly) — else the bare firemark lede when the heat has
                    // no actionable pace.
                    ...
                    // discards the mount output: no gazette, no emblem.
            ...
            JJRM_CMD_NAME_SHOW => {
                ...
                // Groom marks the heat firemark (paddock "Emblem and window
                ...
                if code == 0 {
                    ...
                    if p.hark.is_none() {
                        if let Some(ref fm) = groom_firemark {
                            ...
                            // demotes a coronet a prior mount resolved onto this
                            // officium. When a coronet already stands, record nothing
                            ...
                            let groom_marker = if zjjrm_standing_is_coronet(&exchange) {
                                ...
                                Some(zjjrm_resolve_emblem_marker(fm))
                            };
...
";
    ...
    fn standing_is_coronet_discriminates_marker_identity() {
        ...
        // marker, and garbage are all "fillable" — the groom resolves normally.
        let officia = scratch_officia("standing_coronet");
        let marker_path = officia.join(EMBLEM_MARKER_FILE);
        let write = |m: &jjrm_EmblemMarker| {
