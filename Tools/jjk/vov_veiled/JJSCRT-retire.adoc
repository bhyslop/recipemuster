...
//axd_optional
* `size_limit` (integer) — override the commit size guard limit in bytes (default 200KB).
Use when a large trophy exceeds the default guard.
...
. Run size guard with limit from `size_limit` (default 200KB)
...
.. Advise retry with `size_limit` override
