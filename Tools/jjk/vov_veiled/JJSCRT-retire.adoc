...
//axd_optional
* `--size-limit <bytes>` — override the commit size guard limit (default 200KB).
...
. Run size guard with limit from `--size-limit` (default 200KB)
...
.. Advise retry with `--size-limit` override
