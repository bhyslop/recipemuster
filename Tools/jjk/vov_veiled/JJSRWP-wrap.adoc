...
. Create W chalk marker with outcome summary (uses `summary` parameter if provided, else "pace {silks} complete" from gallops)
. Lookahead: walk parent heat's order array for next actionable pace (first with state {jjdpe_rough})
. Emit `AGENT_RESPONSE:` line to stderr:
.. If next pace found: `AGENT_RESPONSE: ₢{coronet} wrapped. Next: {next_silks} (₢{next_coronet}) — \`/clear\` then \`mount {firemark}\``
.. If no next pace: `AGENT_RESPONSE: ₢{coronet} wrapped. All paces complete — \`/clear\` then \`retire {firemark}\``
. Release lock
. Return commit hash
