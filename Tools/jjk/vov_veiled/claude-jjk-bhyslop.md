...
- **JJS0** → `jjqs_studbook/specs/jjk/JJS0_JobJockeySpec.adoc` (Job Jockey specification - main file)
> **JJS\* sub-sheaf entries are intentionally not listed here.** The Job Jockey spec sheaves (aspirant `JJSA*`, command `JJSC*`, revision-control `JJSV*`, and the rest) load on demand, not always. Discipline: to reach any sheaf, read the SpecTop **JJS0** (`jjqs_studbook/specs/jjk/JJS0_JobJockeySpec.adoc`) FIRST — it is the required entry point and indexes them; the sheaves live beside it as `jjqs_studbook/specs/jjk/JJS*.adoc`.
