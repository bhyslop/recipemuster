...
* `--pace <coronet>` — target
{jjdt_coronet}
(accepts with or without `₢` prefix; falls back to
{jjdts_silks}
match if coronet parse fails or pace not found by coronet).
...
{jjdhm_order},
print `[state] silks (coronet)` where silks is derived from `tacks[0].silks`
