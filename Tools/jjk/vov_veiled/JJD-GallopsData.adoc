...
. Transition pace state to complete via
{jjdo_tally}
. Create W chalk marker with outcome summary
. Release lock
. Return commit hash
...
Where `field` is one of: `silks`, `spec`, `direction`, or `paddock`.
...
.. If any file does not exist: exit 1 with "error: file not found: {path}"
