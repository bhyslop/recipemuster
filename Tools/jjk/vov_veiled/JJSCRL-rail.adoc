...
{jjds_exit_uniform} 0 success (including no-op when pace is already in requested position), non-zero error.
...
. Compare new
{jjdhm_order}
against original. If identical (pace is already in requested position), output order and exit 0 with no commit: `jjx_rail: no change — {coronet} is already {position}`
. Otherwise,
{jjdr_save}
. Output new
{jjdhm_order},
