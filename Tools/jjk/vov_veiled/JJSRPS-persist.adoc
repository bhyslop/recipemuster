...
Staged content over the ceiling refuses as an {jjdz_interdictum} — the size gate — and the refusal is typed, not a message: it carries the measured cost and the per-file breakdown so the emitting command can name them, since the emission must stand alone.
An ordinary commit failure stays an ordinary error, distinguishable at every call site from the gating one.
...
. Format full commit message: `jjb:BRAND:IDENTITY:ACTION: message\n\nCo-Authored-By: Claude Opus 4.5 <noreply@anthropic.com>`
. Run `git commit -m "..."`
. Return commit hash
...
{jjdo_redocket},
{jjdo_curry}
— heat-affiliated discussion commits with action `d`.
{jjdo_redocket}
lands a whole mixed batch — paddock revision, docket revisions, and new
{jjdpr_pace_s}
— under one commit;
{jjdo_curry}
co-commits the paddock file alongside
{jjdgr_gallops}
under the
{jjdhr_heat_p}
{jjdt_firemark}.
...
{jjdo_landing},
{jjdo_chalk}
— marker commits (event records, not content commits). Use the
`vvc::marker` primitive: index-invariant (skips staging), guard-free
(skips size check), and minted via plumbing (`git commit-tree HEAD^{tree} -p HEAD`
+ `git update-ref HEAD`) so any pre-staged content survives untouched in
the index. Markers carry no payload; their purpose is to record an event
in git history, not to author tree content.
*
{jjdo_record}
