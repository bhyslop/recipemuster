...
// jjsuv_: Upper API verbs (horse vocabulary)
...
// Upper API: verbs (horse vocabulary)
...
The display and output layer uses distinct terms for certain schema fields to avoid collision with other project vocabulary:
...
The *upper layer* is the horse racing vocabulary that humans use to direct a language model ({jjsuv_slate}, {jjsuv_mount}, {jjsuv_wrap}, etc.).
...
The human speaks horse vocabulary; the LLM invokes {jjdx_tool_s} via MCP.

=== Verbs

Upper API verbs map horse racing vocabulary to {jjdx_tool_s}.
