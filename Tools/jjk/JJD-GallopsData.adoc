...
All access to the
{jjdgr_gallops}
goes through jjr, the Rust CLI application.
...
----
lock_acquire
jjr gallops nominate --file gallops.json "my-heat" "260111" || { lock_release; die; }
lock_release
----
...
When adding a new
{jjdkr_tack}
via jjr, if state is not explicitly provided, jjr auto-fills from the previous
{jjdkr_tack_p}
state (or
{jjdpe_pending}
for the initial
