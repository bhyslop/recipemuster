...
* `ACTION` — single-letter code (optional for standard notch commits)

*Standard notch* (created by {jjdo_notch}):
----
jjb:BRAND:₢XXXXX: commit message

Co-Authored-By: ...
----

No action code for standard commits. The {jjdt_coronet} provides full context.
...
Field `action` is present for chalk/heat operations, absent for standard notch commits.
...
. Run `git log --all --extended-regexp --grep="^jjb:BRAND:(₣XX|₢XX)" --format="%ai%x09%s"`
...
. Format full message: `jjb:BRAND:₢XXXXX: message\n\nCo-Authored-By: ...`
. Run `git commit -m "..."`
. Release lock
. Output commit hash
