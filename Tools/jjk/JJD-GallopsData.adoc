...
Includes paddock content and full steeplechase history from git.
Use before squash merge to preserve session history.
...
{jjds_stdout} JSON object containing complete archive:
[source,json]
----
{
  "firemark": "₣XX",
...
----
The `paces` array is ordered per
{jjdhm_order}
with full
{jjdkr_tack}
history for each pace.
Field `direction` appears only on
{jjdkr_tack_s}
with state
{jjdpe_primed}.
The `steeplechase` array contains all session history entries (from
{jjdo_rein}).
...
. Build `paces` array by walking
{jjdhm_order}:
.. For each
{jjdt_coronet},
include
{jjdpm_silks}
and full `tacks` array (all history)
. Call
{jjdo_rein}
internally to get steeplechase entries
. Output JSON object with all fields
...
{jjdhr_heat}
status for project review.
Shows paddock, all
{jjdpr_pace_s}
in order, with tack details for actionable paces.
...
// ⟦axd_optional⟧
* `--full` — include tack details for
{jjdpe_complete}
and
{jjdpe_abandoned}
paces (default: silks only)
...
or
{jjdpe_primed}
paces: includes `tack_text` (and `tack_direction` if primed).
For
{jjdpe_complete}
or
{jjdpe_abandoned}
paces: includes only `silks` and `state` unless `--full` flag is set.
Paces ordered per
{jjdhm_order}.
...
exists with given
{jjdt_firemark}
. Output heat metadata: `heat_silks`, `heat_created`, `heat_status`
. Output `paddock_file` from
{jjdhm_paddock}
. Read paddock file contents, output as `paddock_content`
. Build `paces` array by walking
...
.. For each
{jjdt_coronet},
read `tacks[0]` to get current state
.. Output `coronet`, `silks`, `state`
.. If state is
{jjdpe_rough}
or
{jjdpe_primed}:
output `tack_text` (and `tack_direction` if primed)
.. If state is
{jjdpe_complete}
or
{jjdpe_abandoned}
