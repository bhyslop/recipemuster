...
Bash handles locking via `git update-ref` and file orchestration.
jjr is a pure filter: stdin JSON in, stdout JSON out.
If jjr crashes mid-transform, no partial writes occur.
Bash detects the failure and releases the lock.

The pattern:
[source,bash]
----
lock_acquire
jjr transform < studbook.json > studbook_new.json || { lock_release; die; }
mv studbook_new.json studbook.json
lock_release
----
...
Operations are implemented in jjr as pure stdin/stdout filters.
Bash handles file I/O, locking, and orchestration.
