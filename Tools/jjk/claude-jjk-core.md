...
jjx_create         {silks}
jjx_enroll         {firemark, before?, after?, first?, size_limit?} # silks+docket via gazette_in.md only, + jjezs_dictation/jjezs_precis intent companions (see Slating ceremony)
...
| `jjx_enroll` | `# jjezs_slate <silks>` + docket body, plus the intent companions `# jjezs_dictation <silks>` and `# jjezs_precis <silks>` (same lede as the slate — see **Slating ceremony** below) | `{"firemark": "XX", "before?": ..., "after?": ..., "first?": ...}` |
...
**Slating ceremony — original intent (`jjezs_dictation` / `jjezs_precis`).**
Every ceremony slate (`slate` / `chivvy` / `cantle` via `jjx_enroll`) stages TWO
companion notices beside the `jjezs_slate` notice in the same `gazette_in.md`,
each carrying the **same lede** (the silks) as the slate:

- `# jjezs_dictation <silks>` + body — the operator's raw words/context that
  motivated this pace, **verbatim** from the conversation. Quote, never
  paraphrase: this is the authenticity anchor a future reader audits the
  precis against.
- `# jjezs_precis <silks>` + body — your distillation of that context into
  readable intent. It displays as LLM-authored; write it as your best honest
  reading, not a restatement of the docket.

These freeze at slate and are never writable again — populate them ONLY here,
at slating. Reslate/redocket revise the docket alone (and bump the pace's
redocket counter, the drift signal mount reports); do not stage companions on
a reslate, and do not stage them beside `jjx_redocket` batch slates — the
batch vocabulary rejects them loud (batch-born paces carry no intent capture
for now, a documented follow-on). The tool accepts a slate without companions
(old flows still work); the ceremony does not — absent companions on a
ceremony slate is a ceremony failure, not a style choice.
...
3. **Read the paddock before the docket.** The paddock tells you the shape of the work; the docket tells you what to do next. When the orient output carries an `Original-intent` block above the docket, read it as the slate-frozen understanding — audit the precis against the dictation — and treat the docket below as the living authority (the block's staleness caveat and redocket count are the honest drift signals).
