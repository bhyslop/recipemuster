...
#
# Recipe Bottle Regime Update - GCB image pin refresh

set -euo pipefail

# Multiple inclusion detection
test -z "${ZRBRU_SOURCED:-}" || buc_die "Module rbru multiply sourced - check sourcing hierarchy"
ZRBRU_SOURCED=1

######################################################################
# Internal Functions (zrbru_*)
...
zrbru_sentinel() {
  test "${ZRBRU_KINDLED:-}" = "1" || buc_die "Module rbru not kindled - call zrbru_kindle first"
}

######################################################################
# Public Functions (rbru_*)
...
rbru_refresh_gcb_pins() {
  ...
  local z_oras_token_file="${BURD_TEMP_DIR}/rbru_oras_token.json"
  local z_oras_tags_file="${BURD_TEMP_DIR}/rbru_oras_tags.json"
  local z_oras_token_value_file="${BURD_TEMP_DIR}/rbru_oras_token_value.txt"
  local z_oras_tag_file="${BURD_TEMP_DIR}/rbru_oras_tag.txt"
  local z_refresh_prefix="${BURD_TEMP_DIR}/rbru_refresh_"
  local z_refresh_sed_prefix="${BURD_TEMP_DIR}/rbru_refresh_sed_"
