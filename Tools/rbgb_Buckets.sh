...
test -z "${ZRBGB_SOURCED:-}" || bcu_die "Module rbgb multiply sourced - check sourcing hierarchy"
ZRBGB_SOURCED=1
...
zrbgb_kindle() {
  test -z "${ZRBGB_KINDLED:-}" || bcu_die "Module rbgb already kindled"
  ...
  ZRBGB_PREFIX="${BDU_TEMP_DIR}/rbgb_"
...
zrbgb_sentinel() {
  test "${ZRBGB_KINDLED:-}" = "1" || bcu_die "Module rbgb not kindled - call zrbgb_kindle first"
}
...
rbgb_bucket_create() {
  ...
  bcu_doc_brief "Create a Cloud Storage bucket"
  bcu_doc_param "bucket_name" "Name of the bucket to create"
  bcu_doc_shown || return 0

  test -n "${z_bucket_name}" || bcu_die "Bucket name required"

  bcu_step "Creating Cloud Storage bucket: ${z_bucket_name}"

  bcu_log_args 'Get OAuth token from admin'
  local z_token
  z_token=$(rbgu_get_admin_token_capture) || bcu_die "Failed to get admin token"

  bcu_log_args 'Create bucket request JSON'
  local z_bucket_req="${BDU_TEMP_DIR}/rbgb_bucket_create_req.json"
...
rbgb_bucket_get() {
  ...
  bcu_doc_brief "Get Cloud Storage bucket details"
  bcu_doc_param "bucket_name" "Name of the bucket to retrieve"
  bcu_doc_shown || return 0

  test -n "${z_bucket_name}" || bcu_die "Bucket name required"

  bcu_step "Getting Cloud Storage bucket: ${z_bucket_name}"

  bcu_log_args 'Get OAuth token from admin'
  local z_token
  z_token=$(rbgu_get_admin_token_capture) || bcu_die "Failed to get admin token"

  bcu_log_args 'Get bucket via REST API'
  ...
  if rbgu_http_code_capture "${ZRBGB_INFIX_GET}" | grep -q "404"; then
    bcu_info "Bucket not found: ${z_bucket_name}"
    return 1
  fi

  bcu_success "Bucket found: ${z_bucket_name}"
  return 0
}
...
rbgb_bucket_set_iam() {
  ...
  bcu_doc_brief "Set IAM policy on a Cloud Storage bucket"
  bcu_doc_param "bucket_name" "Name of the bucket"
  ...
  bcu_step "Setting IAM policy on bucket: ${z_bucket_name}"

  bcu_log_args 'Get OAuth token from admin'
  local z_token
  z_token=$(rbgu_get_admin_token_capture) || bcu_die "Failed to get admin token"

  bcu_log_args 'Set IAM policy'
  local z_iam_url="${RBGC_API_ROOT_STORAGE}${RBGC_STORAGE_JSON_V1}/b/${z_bucket_name}/iam"
  local z_policy_file="${BDU_TEMP_DIR}/rbgb_bucket_set_iam.json"
  ...
  bcu_success "IAM policy set on bucket: ${z_bucket_name}"
}
...
rbgb_bucket_add_iam_role() {
  ...
  bcu_doc_brief "Add IAM role to a member on a Cloud Storage bucket"
  bcu_doc_param "bucket_name" "Name of the bucket"
  bcu_doc_param "member" "Member to grant role to (serviceAccount:email or user:email)"
  bcu_doc_param "role" "Role to grant (e.g., roles/storage.objectViewer)"
  bcu_doc_shown || return 0
  ...
  bcu_step "Adding IAM role ${z_role} to ${z_member} on bucket: ${z_bucket_name}"

  bcu_log_args 'Get OAuth token from admin'
  local z_token
  z_token=$(rbgu_get_admin_token_capture) || bcu_die "Failed to get admin token"

  bcu_log_args 'Use rbgi_add_bucket_iam_role'
  rbgi_add_bucket_iam_role "${z_token}" "${z_bucket_name}" "${z_member}" "${z_role}"

  bcu_success "Added IAM role ${z_role} to ${z_member} on bucket: ${z_bucket_name}"
}
...
rbgb_bucket_set_lifecycle() {
  ...
  bcu_doc_brief "Set lifecycle policy on a Cloud Storage bucket"
  bcu_doc_param "bucket_name" "Name of the bucket"
  bcu_doc_param "lifecycle_json" "Lifecycle policy JSON (from file or string)"
  bcu_doc_shown || return 0

  test -n "${z_bucket_name}" || bcu_die "Bucket name required"
  test -n "${z_lifecycle_json}" || bcu_die "Lifecycle JSON required"

  bcu_step "Setting lifecycle policy on bucket: ${z_bucket_name}"

  bcu_log_args 'Get OAuth token from admin'
  local z_token
  z_token=$(rbgu_get_admin_token_capture) || bcu_die "Failed to get admin token"

  bcu_log_args 'Set lifecycle policy'
  local z_lifecycle_url="${RBGC_API_ROOT_STORAGE}${RBGC_STORAGE_JSON_V1}/b/${z_bucket_name}"
  local z_lifecycle_file="${BDU_TEMP_DIR}/rbgb_bucket_set_lifecycle.json"
  ...
  bcu_success "Lifecycle policy set on bucket: ${z_bucket_name}"
}
...
rbgb_bucket_delete() {
  ...
  bcu_doc_brief "Delete a Cloud Storage bucket"
  bcu_doc_param "bucket_name" "Name of the bucket to delete"
  bcu_doc_param "force" "If 'true', empty bucket before deletion (optional, default: false)"
  bcu_doc_shown || return 0

  test -n "${z_bucket_name}" || bcu_die "Bucket name required"

  bcu_step "Deleting Cloud Storage bucket: ${z_bucket_name}"
  ...
  if test "${z_force}" = "true"; then
    bcu_log_args 'Empty bucket before deletion'
  ...
  case "${z_code}" in
    204) bcu_success "Bucket ${z_bucket_name} deleted";                           return 0 ;;
    404) bcu_info    "Bucket ${z_bucket_name} not found (already deleted)";       return 0 ;;
    409) bcu_warning "Bucket ${z_bucket_name} not empty or has retention policy"; return 1 ;;
    *)   bcu_warning "Bucket ${z_bucket_name} failed delete";                     return 1 ;;
  esac
}
