...
// NO quoins: the three operation quoins (vosor_release, vosoi_install, vosou_uninstall)
// and every term these procedures consume are defined and registered
// at the cosmology top. This file carries only the procedure bodies — the Obscura
// release/install/uninstall machinery — that consume that vocabulary.
...
CLAUDE.md is never written by
{vosoi_install}:
the operator hand-adds one `@`-include line per public guidance file (`claude-{kit}-*.md`) of each installed
{vose_kit},
per the
{vose_kit_p}
README.
...
{vose_kit}
directories.
Unveiled kit content is typically open-source and remains functional outside Claude Code; vacate removes only the Claude-specific layer (commands, hooks, vvx binary).
...
its hand-maintained `@`-include lines continue to resolve against the preserved
{vose_kit}
directories.
