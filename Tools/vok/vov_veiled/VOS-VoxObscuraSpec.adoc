...
+
Declared as const in `Tools/vok/vof/src/vofc_registry.rs` — the single source of truth for all project prefixes.
+
Declaration pattern:
+
[source,rust]
----
// Tools/vok/vof/src/vofc_registry.rs - Cipher registry (const declarations)
...
copy
{vose_brand_file}
from parcel root to `.vvk/vvbf_brand.json`
.. Brand file contains
{vosem_brand_kits}
for
{vosou_uninstall}
...
expand to
{vose_marker_s}
at that location with template content
... Else (no markers):
{vosc_call}
append
{vose_marker_s}
with template content at end of file
.. {vosc_step}
"CLAUDE.md sections updated"
+
...
replace
{vose_marker_s}
and content between them with single
{vose_uninstalled_marker}
+
...
* Managed sections replaced with {vose_uninstalled_marker} placeholders
* Removed brand file
...
Tools/vok/vof/
  Cargo.toml
  src/
    lib.rs           # Re-exports vofc_registry
    vofc_registry.rs # Cipher type and all cipher constants
----
...
[source,rust]
----
// Tools/vok/vof/src/vofc_registry.rs
...
pub const ALL_CIPHERS: &[vofc_Cipher] = &[VV, VO, JJ, BU, CM, ...];
----
