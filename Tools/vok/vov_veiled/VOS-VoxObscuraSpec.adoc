...
... Route files matching `{cipher}c-*.md` to `.claude/commands/`
... Route files matching `{cipher}h_*` to `.claude/hooks/`
.. {vosc_step}
"{n} kits installed, {m} commands routed, {p} hooks routed"
...
{vose_kit}
directories.
Kits are open-source utilities usable outside Claude Code; vacate removes only the Claude-specific layer (commands, hooks, managed sections, vvx binary).
...
These contain open-source utilities usable outside Claude Code.
...
* Kit directories *preserved* (open-source utilities remain usable)
* Single git commit
