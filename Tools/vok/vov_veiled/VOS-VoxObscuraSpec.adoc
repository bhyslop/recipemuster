...
.. {vosc_call}
resolve paths relative to burc.env parent directory (not CWD)
.. {vosc_store}
absolute paths for tools_dir and project_root
...
.. For each kit in brand's
{vosem_brand_kits}:
... If `${BURC_TOOLS_DIR}/{kit}/` exists:
{vosc_call}
delete recursively
... If absent: log warning, continue (may have been manually removed)
.. {vosc_step}
"{n} kit directories removed"
...
* Kit directories removed from `${BURC_TOOLS_DIR}/`
