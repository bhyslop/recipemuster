...
Whisper::for_kit("jjk")
    ...
    .concept_model("JJS0_JobJockeySpec.adoc")
...
{vosp_kit_forge}
when template content has changed and CLAUDE.md needs to reflect those changes without a full
{vosor_release}
and
{vosoi_install}
cycle.
...
{vose_managed_section}:
{vosc_call}
read template from `${BURC_TOOLS_DIR}/{kit}/vov_veiled/{template_path}`
... {vosc_fatal}
if template not found: "Template not found in forge: {path}"
...
... Else: append at end of file
.. {vosc_step}
report each section as updated, expanded, or appended
...
CLAUDE.md
{vose_managed_section_s}
reflect current forge template content.
