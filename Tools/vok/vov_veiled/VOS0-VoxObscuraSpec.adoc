...
// Matricula vocabulary (transient inscription census; method prose in VMS-VoxMatriculaSpec.adoc)
...
The Vox Specification is the cosmology top for the Vox naming system — the `vo` cipher as a whole.
...
Its branches are the Obscura distribution machinery (release, install, uninstall, freshen — see VOSO-distribution.adoc), the commit/lock routine family (the VOSR* subdocs), and the Matricula transient inscription census (VMS-VoxMatriculaSpec.adoc).
...
The VOF crate (Vox Foundation) is the single source of truth for all project prefixes across the entire VO/VV ecosystem.
...
Its vocabulary is defined here at the cosmology top; its method, decomposition, invariants, and scan mechanics live in the consuming subdoc VMS-VoxMatriculaSpec.adoc, included below.
...
include::VMS-VoxMatriculaSpec.adoc[leveloffset=+1]
