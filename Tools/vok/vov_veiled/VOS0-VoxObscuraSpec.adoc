...
//axl_voices axt_string
{vost_officium}::
Defined by JJK — see `jjdxo_officium` in JJS0_JobJockeySpec.adoc.
A bounded MCP session with unique identity, dedicated filesystem directory, and process-lifetime scope.
...
//axvo_procedure axd_transient axd_internal
{voscai_invitatory}::
Defined by JJK — see `jjdxo_invitatory` in JJS0_JobJockeySpec.adoc.
Opens an {vost_officium} at MCP startup.
