...
Its branches are the Obscura distribution machinery (release, install, uninstall — see VOSO-distribution.adoc), the commit/lock routine family (the VOSR* subdocs), and the Matricula transient inscription census (VOSMM-entity.adoc).
...
The
{vosou_uninstall}
operation removes Claude Code integration (commands, hooks, vvx binary) while preserving
{vose_kit}
directories.
Uses
{vosls_signet}
-based prefix matching for command/hook removal.
The target repo's CLAUDE.md is hand-maintained:
its `@`-include lines for kit guidance files are added by the operator at install and left in place at uninstall,
where they continue to resolve against the preserved
{vose_kit}
directories.
...
The three Obscura distribution operations are defined here at the cosmology top; their procedure bodies live in the consuming subdoc VOSO-distribution.adoc, included below.
...
{vosou_uninstall}::
`vofe_vacate` —
Removes Claude Code integration (commands, hooks, the vvx binary) while preserving
{vose_kit}
directories; uses
{vosls_signet}
-based prefix matching for command/hook removal.
...
Declaration order is significant: kits install in table order.
