...
// Operations
:voso_release:          <<voso_release,Release>>
:voso_install:          <<voso_install,Install>>
:voso_uninstall:        <<voso_uninstall,Uninstall>>
...
Steps:
+
1. Pre-install git commit (if working tree dirty)
2. Copy platform binaries from `bin/` to `Tools/vvk/bin/`
3. Copy
{vose_kit}
assets from `kits/` preserving structure
4. For each
{vose_managed_section}:
replace content between
{vose_marker_s}
in CLAUDE.md
5. Write manifest to `.claude/vvx-manifest.json`
6. Post-install git commit
7. Diff analysis for user modification recovery
