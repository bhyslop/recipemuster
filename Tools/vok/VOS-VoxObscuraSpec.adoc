...
+
Structure: `{signet}{separator}{epithet}{envelope}`
+
...
... `vvi_install.sh` — install script
... `vvu_uninstall.sh` — uninstall script (copied to target)
... `bin/` — platform binaries
... `kits/` — kit assets by directory
...
.. {vosc_call}
copy matching binary from `bin/` to `${BURC_TOOLS_DIR}/vvk/bin/vvx`
.. {vosc_call}
copy `vvu_uninstall.sh` to `${BURC_TOOLS_DIR}/vvk/`
...
* Platform binary at `${BURC_TOOLS_DIR}/vvk/bin/vvx`
