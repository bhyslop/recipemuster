...
rbgp_payor_install() {
  ...
  # Step 2: Validate JSON Key File Format
  ...
  # Step 3: Validate Service Account is Payor Role
  ...
  # Step 4: Generate RBRA File Path
  ...
  # Step 5: Convert JSON to RBRA Format
  bcu_step 'Convert JSON key to RBRA format'
  local z_temp_rbra="${BDU_TEMP_DIR}/payor_install_rbra.tmp"
  
  # Create RBRA file following RBAGS specification
  cat > "${z_temp_rbra}" <<EOF
# Recipe Bottle Role Authentication (RBRA) File
# Generated by rbgp_payor_install on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
# Payor Role Credentials for Project: ${z_project_id}

RBRA_ROLE="payor"
RBRA_PROJECT_ID="${z_project_id}"
RBRA_CLIENT_EMAIL="${z_client_email}"
RBRA_JSON_KEY_DATA="$(jq -c . "${z_json_key_file}" | sed 's/"/\\"/g')"
EOF

  # Step 6: Validate Generated RBRA
  bcu_step 'Validate generated RBRA file'
  test -s "${z_temp_rbra}" || bcu_die "Generated RBRA file is empty"
  grep -q "RBRA_ROLE=\"payor\"" "${z_temp_rbra}" || bcu_die "RBRA file missing payor role declaration"

  # Step 7: Move RBRA to Output Location
  bcu_step 'Install RBRA file to output directory'
  mv "${z_temp_rbra}" "${z_rbra_file}" || bcu_die "Failed to install RBRA file"
