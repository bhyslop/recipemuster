// rbee.EbpfEgress.c - Handles traffic FROM BOTTLE container
// Rewrites destination gateway->sentry for all outbound traffic
...
SEC("tc")
int bottle_egress(struct __sk_buff *skb)
{
    ...
    if (bpf_ntohs(eth->h_proto) == ETH_P_ARP) {
        ...
        // Only handle ARP for gateway IP
        if (arp->ar_tip != RBE_GATEWAY_IP)
            return TC_ACT_SHOT;
            
        // Rewrite to sentry
        __u8 sentry_mac[] = RBE_SENTRY_MAC;
        arp->ar_tip = RBE_SENTRY_IP;
        __builtin_memcpy(eth->h_dest, sentry_mac, ETH_ALEN);
        
        if (bpf_ntohs(arp->ar_op) == ARP_REPLY) {
            __builtin_memcpy(arp->ar_sha, sentry_mac, ETH_ALEN);
    ...
    if (bpf_ntohs(eth->h_proto) == ETH_P_IP) {
        ...
        // Check destination
        if (ip->daddr == bpf_htonl(RBE_GATEWAY_IP)) {
            // Rewrite to sentry
            __u8 sentry_mac[] = RBE_SENTRY_MAC;
            __builtin_memcpy(eth->h_dest, sentry_mac, ETH_ALEN);
        ...
        // Drop traffic to other destinations (security)
        return TC_ACT_SHOT;
    }
