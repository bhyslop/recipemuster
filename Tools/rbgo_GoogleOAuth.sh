...
zrbgo_kindle() {
  ...
  # Default scope for all Google Cloud services
...
zrbgo_base64url_encode_capture() {
  ...
  # Convert to URL-safe: + to -, / to _, remove =
...
rbgo_get_token_capture() {
  ...
  # Validate lifetime bounds (Google allows 300-3600)
  test "${z_lifetime_seconds}" -ge 300 || return 1
  test "${z_lifetime_seconds}" -le 3600 || return 1

  # Source service account key path from env file
  # Note: This sources into function scope only, vars don't leak
  source "${z_service_env_file}" || return 1

  # Find which key variable was set
  local z_key_json
